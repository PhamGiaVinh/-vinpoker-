...
3. Verify the global policy row has `future_club_enabled=false`, all direct
...
4. With a TEST super-admin session, call the global RPC inside a transaction
...
6. UAT one open PT dealer, one historical checked-out PT dealer, an idempotent

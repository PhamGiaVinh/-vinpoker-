...
### Amendments (2026-06-26, owner review during Patch 5 — HYBRID trigger model)
- **A8.0 — model (supersedes the inline approach for the auto-swing path).** Live `pg_proc` introspection proved `perform_swing` has 3 live overloads (a CORE seat-writer, a self-picking WRAPPER that delegates to CORE, and a dead 5-arg seat-writer) plus `execute_pre_assigned_swing`. Rather than reproduce 3–4 complex live rotation RPCs inline, auto-swing pool enforcement is **one `BEFORE INSERT OR UPDATE` trigger** (`public.dealer_assignments_pool_enforce`) at the shared seat sink — it covers every current and future seat-writer, both INSERT-new and promote-to-active UPDATE (`reserved`/`pre_assigned` → `assigned`), and skips already-active bookkeeping + race-restores of the same dealer/table. A7's inline "profile-first" remains the rule for any future inline enforcement; the trigger is the sanctioned exception with the A8.2 proof.
- **A8.1 — kill-switch truthiness (canonical).** `app_settings('dealer_feature_tables_enabled')` is a JSONB boolean (seed `'false'::jsonb`). SQL reads `value = 'true'::jsonb` (helper `_assert` + the trigger's early short-circuit); the Patch-5b TS picker reads `value === true`; flipping ON sets `'true'::jsonb`. The trigger short-circuits (no profile lock, no work) when off → fully inert.
- **A8.2 — lock order + deadlock proof.** The trigger acquires `dealer_table_profiles FOR UPDATE` at the seat write and holds it through commit → a concurrent `set_table_dealer_pool` (which locks the same row) cannot change the pool between the check and the seat (TOCTOU closed). Order is `{dealer_assignments | dealer_attendance} → dealer_table_profiles` **uniformly** — Patch 5a removes `assign_dealer_to_table`'s former profile-first lock (STEP 0.7) so no path locks profile-then-da/attendance. Deadlock-free: config RPCs lock ONLY the profile row; every seat-writer locks its own da/attendance up front then the profile last; `assign_dealer_to_table` takes attendance with `SKIP LOCKED` (never waits); so no transaction holding the profile waits on a da/attendance held by a profile-waiter.
- **A8.3 — override skip-flag + its precondition (INVARIANT).** `assign_dealer_to_table`'s authorized+audited override sets tx-local `dealer_feature.override='on'`; the trigger stands down for that write. This is forgery-proof **only because** Patch 5a revokes the direct table write grants on `dealer_assignments` from `authenticated`/`anon` (the four write privileges; details in the migration) → the sole non-service_role writer is a `postgres` SECURITY DEFINER RPC. **If any future migration re-grants those writes to `authenticated`/`anon`, the skip-flag becomes a bypass — the revoke must hold.** (Verified safe to revoke: the only direct write in code is `process-swing` via service_role; the frontend uses RPCs and keeps SELECT.)
- **A8.4 — flip coupling (5a ↔ 5b).** Flip the kill-switch ON only with Patch 5b live, else the cron/WRAPPER keeps feeding non-pool dealers → trigger-rollback every cycle → the outgoing dealer is stuck on the seat with OT/priority accruing. Flip only after 5a + 5b + Patch 6.
- **A8.5 — shortage signal + accrual caveat.** The CLEAN shortage path is Patch 5b's picker returning null for a special table (no eligible pool dealer) → the existing cron `no_dealer` keep-seat (which commits OT/priority accrual) + an operator alert pointing to the remedy (add a pool dealer / floor manual override) + a runbook entry. The trigger-rollback is the rare race/bug backstop; being a rollback it returns `error` and does NOT persist OT/priority accrual — so paths not covered by 5b's picker (the WRAPPER's SQL self-pick, the frontend manual swing on a special table, the dead 5-arg) lose accrual on shortage until Patch 6 hardens them. Documented, not implied-clean.

-- ════════════════════════════════════════════════════════════════════════════
-- ROLLBACK snapshot for migration 20260924000000_perform_swing_racelost_rollback_identity
--
-- EXACT live body of perform_swing(p_assignment_id uuid, p_version integer,
-- p_next_attendance_id uuid, p_send_to_break boolean, p_break_duration_minutes integer,
-- p_swing_duration_minutes integer, p_swing_due_at timestamptz, p_rest_deficit_minutes
-- integer) [oid 199044] captured via pg_get_functiondef on 2026-06-18 (Phase-0 probe
-- #311). To roll back the controlled apply: re-run this CREATE OR REPLACE.
--
-- Difference vs the patched version: the race-lost rollback below restores ONLY
-- current_state / priority_break_flag (guessed) / overtime_minutes (arithmetic) — it
-- does NOT restore last_released_at or worked_minutes_since_last_break (the bug PR3-C
-- fixes). Rolling back reinstates that bug — only do so on a regression.
-- ════════════════════════════════════════════════════════════════════════════

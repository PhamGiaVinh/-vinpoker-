-- PATCH I rollback snapshot — captured 2026-06-11 BEFORE applying migration
-- 20260812000000_patch_i_perform_swing_active_conflict.sql
--
-- Target OID 199044:
...
--   This is the INNER overload delegated to by the dispatcher overload
--   perform_swing(p_assignment_id, p_duration_minutes, ...) OID 199042, which is
--   what the process-swing edge function calls. Overload 257642 is not reachable
--   from the edge function.
--
-- This is the EXACT live body via SELECT pg_get_functiondef(199044::oid)
-- run immediately before PATCH I. Differences vs PATCH I:
--   - no Fix A stale on_break release before the INSERT
--   - ON CONFLICT predicate is the OLD subset form WHERE (status = 'assigned')
--   - no dealer_breaks row created when p_send_to_break = true
--   - no EXCEPTION handler (duplicate-key propagates as raw RPC error)
--
-- To revert PATCH I: run this file via the Management API SQL executor.
-- ⚠️ Reverting restores the duplicate-key crash exposure on the fallback swing path.

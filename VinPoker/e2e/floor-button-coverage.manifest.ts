...
export const floorButtonCoverageManifest: readonly FloorButtonCoverageEntry[] = [
  ...
  { id: "tables-player-row", route: "/ops/tables", role: "floor", viewport: all, label: "Người chơi TEST thuộc bàn", labelPattern: "^[0-9]+\\s+codex_floor_canary_[0-9]{14}_[a-f0-9]{8}_table_lifecycle_(?:p[0-9]+|added)\\s+[0-9.,]+$", expectedState: "navigation-only", expectedBackendCall: null, expectedDbInvariant: "the player row belongs to the exact-owned table", fixtureScenario: "TABLE_LIFECYCLE", destructive: false },
  ...
  { id: "payout-style", route: "/floor", role: "owner", viewport: all, label: "Kiểu giải", labelPattern: "^(daily \\(top nặng · 2×\\)|intl \\(phẳng · 2×\\)|multi \\(phẳng · 1\\.5×\\)|triton \\(tham khảo\\)|custom — clb tự cấu hình|live standard \\(final table riêng · ngoài ft theo nhóm\\))$", expectedState: "navigation-only", expectedBackendCall: null, expectedDbInvariant: null, fixtureScenario: "PAYOUT_CLOSE", destructive: false },
  ...
  { id: "payout-satellite-remove-row", route: "/floor", role: "owner", viewport: all, label: "Xoá dòng", labelPattern: "^xoá dòng \\d+$", expectedState: "enabled", expectedBackendCall: null, expectedDbInvariant: "the added browser-only draft row is removed and satellite_payout remains null", fixtureScenario: "PAYOUT_CLOSE", destructive: false },

...
DO $$ BEGIN
  IF current_setting('server_version_num')::int NOT BETWEEN 170000 AND 179999
     OR current_database() <> 'wave3_timeline_disposable'
...
CREATE TABLE public.dealer_assignments (id uuid PRIMARY KEY, table_id uuid NOT NULL REFERENCES public.game_tables(id), table_session_id uuid, assigned_at timestamptz NOT NULL, released_at timestamptz);
...
INSERT INTO public.tournament_entries VALUES
 ('e0000000-0000-0000-0000-000000000001','aaaaaaaa-0000-0000-0000-000000000001','50000000-0000-0000-0000-000000000001','2026-09-14 08:00+00',null),
 ('e0000000-0000-0000-0000-000000000002','aaaaaaaa-0000-0000-0000-000000000001','50000000-0000-0000-0000-000000000001','2026-09-14 08:10+00',null),
 ('e0000000-0000-0000-0000-000000000003','aaaaaaaa-0000-0000-0000-000000000001','50000000-0000-0000-0000-000000000002','2026-09-14 08:05+00','2026-09-14 08:20+00');
...
INSERT INTO public.dealer_assignments VALUES
 ('90000000-0000-0000-0000-000000000001','60000000-0000-0000-0000-000000000001','70000000-0000-0000-0000-000000000001','2026-09-14 08:15+00','2026-09-14 09:00+00'),
 -- E: relevant table assignment exists, but its session identity is missing.
 ('90000000-0000-0000-0000-000000000003','60000000-0000-0000-0000-000000000003',null,'2026-09-14 08:05+00',null);
...
DO $$ DECLARE v jsonb; BEGIN
  ...
  IF v#>>'{gtd,guaranteeState}' <> 'available' OR v#>>'{gtd,points,1,value}' <> '500' THEN RAISE EXCEPTION 'gtd zero buy-in'; END IF;
  IF v::text ~ '(player|dealer).*(00000000|50000000|90000000)' THEN RAISE EXCEPTION 'identity leak'; END IF;

...
test("accepts unique versioned migrations and ignores non-versioned helpers", () => {
  withCatalog(
    ["20270101000000_first.sql", "20270101000001_second.sql", "_dry_run.sql"],
    (migrations) => {
...
test("rejects every migration file sharing a version", () => {
  withCatalog(
    ["20270101000000_alpha.sql", "20270101000000_beta.sql"],
    (migrations) => {
      assert.deepEqual(findMigrationCatalogProblems(migrations), [
...
test("rejects held Payroll PDF migration in the active catalog", () => {
  withCatalog(
    ...
    (migrations) => {
      assert.deepEqual(findMigrationCatalogProblems(migrations), [
        "forbidden active migration 20270113000000_dealer_payroll_statement_pdf_storage.sql: historical Payroll source is preserved; inspect live lineage before any delta migration",
      ]);
    },
  );
});
...
test("rejects held Payroll FT migration in the active catalog", () => {
  withCatalog(
    ...
    (migrations) => {
      assert.deepEqual(findMigrationCatalogProblems(migrations), [
        "forbidden active migration 20270113000001_dealer_payroll_statement_ft_ui_contract.sql: Payroll migration has alias history; do not replay the archived source",
      ]);
    },
  );
});
...
test("rejects held Payroll Telegram migration in the active catalog", () => {
  withCatalog(
    ...
    (migrations) => {
      assert.deepEqual(findMigrationCatalogProblems(migrations), [
        "forbidden active migration 20270113000004_dealer_payroll_statement_telegram_delivery.sql: Payroll/Telegram migration has alias history; do not replay the archived source",
      ]);
    },
  );
});
    ...
    [
      {
        ...
        source:
          "select 'https://orlesggcjamwuknxwcpk.supabase.co/functions/v1/example';\n",
      },
    ],
    (migrations) => {
      assert.deepEqual(findMigrationCatalogProblems(migrations), [

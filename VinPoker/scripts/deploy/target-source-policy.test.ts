...
const defaultRoot = decodeURIComponent(
  new URL("../../..", import.meta.url).pathname,
).replace(/^\/(.:\/)/, "$1");
const root = Deno.env.get("TARGET_REPO_ROOT") ?? defaultRoot;
const targets = JSON.parse(
  Deno.env.get("TARGET_FUNCTIONS") ??
    '["process-swing","mass-assign","checkout-dealer","tournament-live-clock"]',
) as string[];
const manifestPath = Deno.env.get("CONTROL_PLANE_MANIFEST") ??
  decodeURIComponent(
    new URL("./deployment-contracts.json", import.meta.url).pathname,
  ).replace(/^\/(.:\/)/, "$1");
...
Deno.test("exact target source exposes every selected critical Edge entrypoint", async () => {
  ...
  for (const name of targets) {
    ...
    assert(
      config?.critical,
      `${name} must remain critical in the current manifest`,
    );

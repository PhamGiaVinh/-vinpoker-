...
try {
  ...
  $files = @{
    ...
    '/tmp/v2.sql' = Join-Path $vinPokerRoot 'supabase\migration-archive\superseded\remote-alias\20270106000001_dealer_pt_wage_global_continuous_accrual_v2.sql'
    '/tmp/readiness-acl-repair.sql' = Join-Path $vinPokerRoot 'supabase\migration-archive\superseded\remote-alias\20270106000002_dealer_pt_wage_readiness_acl.sql'
    ...
    '/tmp/payroll-statements-v1.sql' = Join-Path $vinPokerRoot 'supabase\migration-archive\historical-never-replay\20270112000000_dealer_payroll_statements_v1.sql'

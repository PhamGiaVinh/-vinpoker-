-- ============================================================================
-- PATCH 4 B+C — CONTROLLED PRODUCTION APPLY (this script COMMITs — it is the REAL apply).
-- ============================================================================
-- Run ONLY in an owner controlled session, AFTER patch4_bc_preflight.sql shows all PASS.
-- Applies STAGE B + C-confirm + C-settle in ONE transaction; a gate RAISEs (auto-rollback) if any
-- object is missing, so a partial/failed apply COMMITs NOTHING. On success it COMMITs, then prints a
-- read-only 10-row PASS proof. Same DDL as the FULL-PASS dry-run, minus the BEGIN…ROLLBACK wrapper.
--
-- AFTER this: run sepay_reentry_auto_confirm_sandbox_test.sql (BEGIN…ROLLBACK) → expect 9/9 PASS.
-- Do NOT deploy tournament-reentry or flip dynamicReentry until BOTH the proof (10/10) and that test
-- (9/9) pass. See PATCH_4_STAGE_C_APPLY_RUNBOOK.md for rollback/emergency steps.
-- ============================================================================

BEGIN;

-- ===== STAGE B (20261122000000_treg_source_entry_id.sql) =====
...
-- ===== STAGE C helper/confirm (20261122000001_confirm_reentry_and_assign_seat.sql) =====
...
-- ===== STAGE C settle (20261123000000_sepay_settle_reentry_autoconfirm.sql) =====
...
-- ===== APPLY GATE — RAISEs (aborts before COMMIT) if any object is missing/wrong. Fail-safe. =====
DO $$
DECLARE bad text := '';
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='tournament_registrations' AND column_name='source_entry_id') THEN bad := bad || ' source_entry_id'; END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='uniq_treg_active_initial') THEN bad := bad || ' uniq_treg_active_initial'; END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='uniq_treg_pending_reentry_per_entry') THEN bad := bad || ' uniq_treg_pending_reentry_per_entry'; END IF;
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='uniq_treg_active') THEN bad := bad || ' OLD-uniq_treg_active-still-present'; END IF;
  IF to_regprocedure('public._assign_reentry_seat(uuid,uuid,uuid,uuid,uuid,text,integer)') IS NULL THEN bad := bad || ' _assign_reentry_seat'; END IF;
  IF to_regprocedure('public.confirm_reentry_and_assign_seat(uuid,uuid,text)') IS NULL THEN bad := bad || ' confirm_reentry_and_assign_seat'; END IF;
  IF pg_get_functiondef(to_regprocedure('public.reenter_tournament_player(uuid,bigint,bigint,text)')) !~ '_assign_reentry_seat' THEN bad := bad || ' reenter-not-calling-helper'; END IF;
  IF pg_get_functiondef(to_regprocedure('public.settle_bank_transaction(uuid,boolean)')) !~ 'confirm_reentry_and_assign_seat' THEN bad := bad || ' settle-dispatch'; END IF;
  IF pg_get_functiondef(to_regprocedure('public.settle_bank_transaction(uuid,boolean)')) !~ 'upper\(v_ref\)\s+LIMIT 1\s+FOR UPDATE' THEN bad := bad || ' settle-FOR-UPDATE'; END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='uniq_payment_settlements_autoconfirm_per_reg') THEN bad := bad || ' uniq_payment_settlements_autoconfirm_per_reg'; END IF;
  IF bad <> '' THEN
    RAISE EXCEPTION 'PATCH4 B+C APPLY GATE FAILED — missing/wrong:%. Transaction will ROLL BACK; nothing applied.', bad;
  END IF;
  RAISE NOTICE 'PATCH4 B+C APPLY GATE PASSED — all 10 objects present. Proceeding to COMMIT.';
END $$;

COMMIT;

-- ===== POST-COMMIT PROOF (READ-ONLY; runs after COMMIT; expect 10 rows = PASS) =====
SELECT 1 AS seq, 'source_entry_id column live' AS check_name,
   CASE WHEN EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='tournament_registrations' AND column_name='source_entry_id') THEN 'PASS' ELSE 'FAIL' END AS result
UNION ALL SELECT 2, 'uniq_treg_active_initial live', CASE WHEN EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='uniq_treg_active_initial') THEN 'PASS' ELSE 'FAIL' END
UNION ALL SELECT 3, 'uniq_treg_pending_reentry_per_entry live', CASE WHEN EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='uniq_treg_pending_reentry_per_entry') THEN 'PASS' ELSE 'FAIL' END
UNION ALL SELECT 4, 'old uniq_treg_active GONE', CASE WHEN NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='uniq_treg_active') THEN 'PASS' ELSE 'FAIL' END
UNION ALL SELECT 5, '_assign_reentry_seat live', CASE WHEN to_regprocedure('public._assign_reentry_seat(uuid,uuid,uuid,uuid,uuid,text,integer)') IS NOT NULL THEN 'PASS' ELSE 'FAIL' END
UNION ALL SELECT 6, 'confirm_reentry_and_assign_seat live', CASE WHEN to_regprocedure('public.confirm_reentry_and_assign_seat(uuid,uuid,text)') IS NOT NULL THEN 'PASS' ELSE 'FAIL' END
UNION ALL SELECT 7, 'reenter calls shared helper', CASE WHEN pg_get_functiondef(to_regprocedure('public.reenter_tournament_player(uuid,bigint,bigint,text)')) ~ '_assign_reentry_seat' THEN 'PASS' ELSE 'FAIL' END
UNION ALL SELECT 8, 'settle dispatches to re-entry confirm', CASE WHEN pg_get_functiondef(to_regprocedure('public.settle_bank_transaction(uuid,boolean)')) ~ 'confirm_reentry_and_assign_seat' THEN 'PASS' ELSE 'FAIL' END
UNION ALL SELECT 9, 'settle reg-select has FOR UPDATE', CASE WHEN pg_get_functiondef(to_regprocedure('public.settle_bank_transaction(uuid,boolean)')) ~ 'upper\(v_ref\)\s+LIMIT 1\s+FOR UPDATE' THEN 'PASS' ELSE 'FAIL' END
UNION ALL SELECT 10, 'uniq_payment_settlements_autoconfirm_per_reg live', CASE WHEN EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='uniq_payment_settlements_autoconfirm_per_reg') THEN 'PASS' ELSE 'FAIL' END
ORDER BY seq;

...
async function sql(label, q) {
  ...
  if (!res.ok) { log(`${label}: HTTP ${res.status} ${mask(text).slice(0, 600)}`); return; }
  log(`── ${label} ──`); console.log(mask(text).slice(0, 5000));
}

// The edge fn inserts ONLY these columns; a NOT NULL col w/o default outside this set → guaranteed 500.
await sql("NOT-NULL cols w/o default NOT in the insert set", `

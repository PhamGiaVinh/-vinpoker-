...
function createPacket(config, database, { event, snapshot = null, state = "no_forecast_available", metric = "entries", horizon = "T-7", key, parent = null }) {
  ...
  const manual = state === "manual_expectation" ? "500" : "NULL";
  const args = [
    `${sql(event)}::uuid`, sql(horizon), sql(metric), sql("2026-01-02T00:00:00.000Z") + "::timestamptz", sql("2026-01-01T00:00:00.000Z") + "::timestamptz", `${sql(TARGET_DATES[event])}::timestamptz`,
    snapshotArg, sql(state), manual, "'[]'::jsonb", "NULL", "NULL", "NULL", "NULL", "NULL", "'{}'::jsonb", "NULL", "NULL", "NULL", "NULL", "NULL", "NULL", "'[]'::jsonb", "'[]'::jsonb", "NULL", parent ? `${sql(parent)}::uuid` : "NULL", parent ? sql("supersedes for acceptance") : "NULL", sql(key)
  ];
...
async function main() {
  ...
  try {
    ...
    check("C05_manual_negative_money_fails", () => expectError(config, database, `SELECT public.series_record_event_actual_v1(${sql(EVENTS.completed)}::uuid,'event_total','final','exact','2026-02-04T00:00:00Z','present',2,'present',1,'present',2,'present',1,'present',2,'missing',NULL,'present',-1,'VND',0,'explicit_zero',0,'VND',0,NULL,${sql("manual:negative:001")},NULL)`, OWNER, /money|availability|invalid/));

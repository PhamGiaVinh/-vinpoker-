...
async function main() {
  ...
  try {
    ...
    check("F04_legacy_cache_is_not_promoted", () => expect(scalar(config, database, `SELECT (SELECT count(*) FROM public.series_event_actuals WHERE event_id=${sql(EVENTS.noForecast)}::uuid) = 1 AND (SELECT count(*) FROM public.series_event_actual_revisions_v1 WHERE event_id=${sql(EVENTS.noForecast)}::uuid) = 0`) === "true", "legacy cache promoted"));

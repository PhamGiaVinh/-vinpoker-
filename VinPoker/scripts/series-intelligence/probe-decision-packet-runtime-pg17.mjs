...
function createManual(config, database, { event, scope = "event_total", finality = "final", source = "2026-02-02T00:00:00.000Z", entries = 2, unique = 1, bullets = 2, reentries = 1, records = 2, paid = null, prize = 600000000, prizeAvailability = "present", prizeCurrency = "VND", prizeScale = 0, overlay = null, overlayAvailability = "explicit_zero", idempotency, parent = null, reason = null }) {
  ...
  const money = (availability, amount, currency, scale) => {
    const normalizedAmount = availability === "explicit_zero" && amount === null ? 0 : amount;
    const normalizedCurrency = availability === "explicit_zero" && currency === null ? "VND" : currency;
    const normalizedScale = availability === "explicit_zero" && scale === null ? 0 : scale;
    return `${sql(availability)}::text,${normalizedAmount === null ? "NULL::numeric" : `${normalizedAmount}::numeric`},${normalizedCurrency === null ? "NULL::text" : `${sql(normalizedCurrency)}::text`},${normalizedScale === null ? "NULL::smallint" : `${normalizedScale}::smallint`}`;
  };
  const args = [
...
INSERT INTO public.series_event_actuals(event_id,club_id) VALUES (${sql(e.noForecast)}::uuid,${sql(CLUB)}::uuid);
...
async function main() {
  ...
  try {
    ...
    packetEligible = createPacket(config, database, { event: EVENTS.supersession, snapshot: SNAPSHOTS.supersession, state: "forecast_identity_eligible", horizon: "T-0", key: "packet:eligible:001" });
    ...
    check("A01_postgres_server_is_17", () => expect(/^17\./.test(scalar(config, database, "SHOW server_version")), scalar(config, database, "SHOW server_version")));
    ...
    check("B09_native_anonymous_call_fails", () => expectError(config, database, `SET ROLE anon; SELECT public.series_promote_native_event_actual_v1(${sql(EVENTS.eligible)}::uuid,${sql("native:eligible:003")})`, null, /unauthenticated|forbidden|permission denied/));
    ...
    check("C02_manual_actual_requires_owner", () => expectError(config, database, `SET ROLE anon; SELECT public.series_record_event_actual_v1(${sql(EVENTS.completed)}::uuid,'event_total'::text,'final'::text,'exact'::text,'2026-02-04T00:00:00Z'::timestamptz,'present'::text,2::bigint,'present'::text,1::bigint,'present'::text,2::bigint,'present'::text,1::bigint,'present'::text,2::bigint,'missing'::text,NULL::bigint,'present'::text,600000000::numeric,'VND'::text,0::smallint,'explicit_zero'::text,0::numeric,'VND'::text,0::smallint,NULL::uuid,${sql("manual:anon:001")}::text,NULL::text)`, null, /unauthenticated|forbidden|permission denied/));
    check("C03_manual_finality_requires_completed_event", () => expectError(config, database, `SELECT public.series_record_event_actual_v1(${sql(EVENTS.noForecast)}::uuid,'event_total'::text,'final'::text,'exact'::text,'2026-02-02T00:00:00Z'::timestamptz,'present'::text,2::bigint,'present'::text,1::bigint,'present'::text,2::bigint,'present'::text,1::bigint,'present'::text,2::bigint,'missing'::text,NULL::bigint,'present'::text,600000000::numeric,'VND'::text,0::smallint,'explicit_zero'::text,0::numeric,'VND'::text,0::smallint,NULL::uuid,${sql("manual:active:001")}::text,NULL::text)`, OWNER, /finality_not_supported/));
    ...
    check("C05_manual_negative_money_fails", () => expectError(config, database, `SELECT public.series_record_event_actual_v1(${sql(EVENTS.completed)}::uuid,'event_total'::text,'final'::text,'exact'::text,'2026-02-04T00:00:00Z'::timestamptz,'present'::text,2::bigint,'present'::text,1::bigint,'present'::text,2::bigint,'present'::text,1::bigint,'present'::text,2::bigint,'missing'::text,NULL::bigint,'present'::text,-1::numeric,'VND'::text,0::smallint,'explicit_zero'::text,0::numeric,'VND'::text,0::smallint,NULL::uuid,${sql("manual:negative:001")}::text,NULL::text)`, OWNER, /money|availability|invalid/));
    ...
    check("C07_manual_foreign_owner_fails", () => expectError(config, database, `SELECT public.series_record_event_actual_v1(${sql(EVENTS.completed)}::uuid,'event_total'::text,'final'::text,'exact'::text,'2026-02-04T00:00:00Z'::timestamptz,'present'::text,2::bigint,'present'::text,1::bigint,'present'::text,2::bigint,'present'::text,1::bigint,'present'::text,2::bigint,'missing'::text,NULL::bigint,'present'::text,600000000::numeric,'VND'::text,0::smallint,'explicit_zero'::text,0::numeric,'VND'::text,0::smallint,NULL::uuid,${sql("manual:foreign:001")}::text,NULL::text)`, OTHER_OWNER, /forbidden/));
    check("C08_manual_correction_requires_owner_reason", () => expectError(config, database, `SELECT public.series_record_event_actual_v1(${sql(EVENTS.completed)}::uuid,'event_total'::text,'corrected'::text,'exact'::text,'2026-02-04T00:00:00Z'::timestamptz,'present'::text,2::bigint,'present'::text,1::bigint,'present'::text,2::bigint,'present'::text,1::bigint,'present'::text,2::bigint,'missing'::text,NULL::bigint,'present'::text,600000000::numeric,'VND'::text,0::smallint,'explicit_zero'::text,0::numeric,'VND'::text,0::smallint,${sql(manualEligible)}::uuid,${sql("manual:bad-correction:001")}::text,NULL::text)`, OWNER, /correction|reason|predecessor/));
    ...
    check("D04_active_truth_chosen_revision_is_reconciled", () => { const result = json(config, database, `SELECT public._series_d2b_actual_truth_state_v1(${sql(EVENTS.eligible)}::uuid)`); expect(result.state === "current" && result.sourceState === "reconciled", JSON.stringify(result)); });
    ...
    check("E01_conflict_record_is_non_scoring", () => { const autoResult = json(config, database, `SELECT public.series_promote_native_event_actual_v1(${sql(EVENTS.conflict)}::uuid,${sql("native:conflict:001")})`, OWNER); autoConflict = autoResult.revision.revisionId; manualConflict = createManual(config, database, { event: EVENTS.conflict, source: "2026-02-06T00:00:00.000Z", entries: 4, unique: 3, bullets: 4, reentries: 1, idempotency: "manual:conflict:001" }); const result = reconcile(config, database, autoConflict, manualConflict, "blocked_conflict", "reconcile:conflict:001"); expect(result.state === "conflict_recorded", JSON.stringify(result)); });
    check("E02_conflict_truth_state_blocks", () => { const result = json(config, database, `SELECT public._series_d2b_actual_truth_state_v1(${sql(EVENTS.conflict)}::uuid)`); expect(result.state === "conflict", JSON.stringify(result)); });
    ...
    check("F04_legacy_cache_is_not_promoted", () => { const counts = scalar(config, database, `SELECT (SELECT count(*) FROM public.series_event_actuals WHERE event_id=${sql(EVENTS.noForecast)}::uuid)::text || ':' || (SELECT count(*) FROM public.series_event_actual_revisions_v1 WHERE event_id=${sql(EVENTS.noForecast)}::uuid)::text`); expect(counts === "1:0", `legacy cache counts ${counts}`); });
    ...
    check("F07_conflicting_revision_never_becomes_current", () => { const result = json(config, database, `SELECT public._series_d2b_actual_truth_state_v1(${sql(EVENTS.conflict)}::uuid)`); expect(result.state === "conflict" && !result.chosenRevision, JSON.stringify(result)); });
    ...
    check("G03_state_read_omits_player_identity", () => { const result = scalar(config, database, `SELECT public.series_get_decision_event_state_v1(${sql(EVENTS.eligible)}::uuid)::text`, OWNER); expect(!/playerId|60000000-0000-4000-8000/i.test(result), result); });
    ...
    check("J04_reported_actual_truth_is_deterministic_shape", () => { const result = json(config, database, `SELECT public.series_get_decision_event_state_v1(${sql(EVENTS.completed)}::uuid)`, OWNER); expect(result.version === "series-decision-event-state-v1" && Array.isArray(result.scoring.blockReasons), JSON.stringify(result)); });
  } finally {

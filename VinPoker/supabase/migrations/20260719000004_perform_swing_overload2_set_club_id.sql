-- =============================================================================
-- Migration: perform_swing (core 7-param overload) — include club_id in INSERT
--
-- Context:
--   After Phase 1 added club_id NOT NULL to dealer_assignments, the core
--   perform_swing overload (7-param with p_swing_due_at) must include
--   club_id in its INSERT INTO dealer_assignments.
--
--   club_id is resolved from game_tables via JOIN in the FOR UPDATE block.
--
-- Signature: (p_assignment_id, p_version, p_next_attendance_id, p_send_to_break,
...
-- Cannot use DROP FUNCTION ... (uuid, int, uuid, bool, int, int, timestamptz)
-- without specifying exact types if there are other overloads. Use the
-- function's full signature to disambiguate.
...
COMMENT ON FUNCTION public.perform_swing(UUID, INTEGER, UUID, BOOLEAN, INTEGER, INTEGER, TIMESTAMPTZ) IS
  'Core 7-param swing engine. Phase 1: includes club_id in INSERT for NOT NULL compliance.';

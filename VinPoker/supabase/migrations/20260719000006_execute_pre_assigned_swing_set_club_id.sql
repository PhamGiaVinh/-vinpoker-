-- =============================================================================
-- Migration: execute_pre_assigned_swing — include club_id in INSERT (both overloads)
--
-- Context:
--   After Phase 1 added club_id NOT NULL to dealer_assignments, this RPC must
--   include club_id in its INSERT INTO dealer_assignments.
--
--   club_id is resolved from game_tables via JOIN (in [1] Get old assignment info).
--
-- Two overloads updated:
--   1. (p_old_assignment_id, p_next_attendance_id, p_swing_due_at,
--       p_duration_minutes, p_send_to_break, p_break_duration_minutes)
--   2. (p_old_assignment_id, p_next_attendance_id, p_swing_due_at,
--       p_duration_minutes, p_break_duration_minutes, p_send_to_break)
--
-- Note: The two overloads differ in argument order for p_send_to_break and
-- p_break_duration_minutes. Both bodies are kept functionally identical
-- except for that swap.
-- =============================================================================
...
COMMENT ON FUNCTION public.execute_pre_assigned_swing(UUID, UUID, TIMESTAMPTZ, INTEGER, BOOLEAN, INTEGER) IS
  'Execute pre-assigned swing (overload 1). Phase 1: includes club_id in INSERT for NOT NULL compliance.';

COMMENT ON FUNCTION public.execute_pre_assigned_swing(UUID, UUID, TIMESTAMPTZ, INTEGER, INTEGER, BOOLEAN) IS
  'Execute pre-assigned swing (overload 2). Phase 1: includes club_id in INSERT for NOT NULL compliance.';

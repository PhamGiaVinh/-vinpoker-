-- Fix perform_swing RPC: compute worked_minutes_since_last_break from real timestamps
-- instead of leaving it stale. Reset to 0 when sending to break.
...
BEGIN
  ...
  IF p_send_to_break THEN
    ...
    SET current_state = 'on_break',
        ...
        worked_minutes_since_last_break = 0  -- reset on break
  ...
  -- Update new dealer's worked_minutes_since_last_break from real timestamps (since last break end or check-in)
  UPDATE dealer_attendance

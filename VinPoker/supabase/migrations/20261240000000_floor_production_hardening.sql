...
GRANT EXECUTE ON FUNCTION public.floor_assign_player_to_seat(UUID, TEXT, UUID, INTEGER) TO authenticated, service_role;
...
DECLARE
  ...
  v_attempt INTEGER := 0;
BEGIN
  IF v_actor IS NULL OR p_actor_user_id IS NULL OR p_actor_user_id IS DISTINCT FROM v_actor THEN
    RETURN jsonb_build_object('ok', false, 'error', 'actor_not_allowed');
  END IF;
...
BEGIN
  ...
  IF NOT v_seat.is_active THEN
    IF v_seat.status = 'busted' THEN
      RETURN jsonb_build_object('ok', true, 'outcome', 'already_busted', 'seat_id', p_seat_id);
    END IF;
...
GRANT EXECUTE ON FUNCTION public.floor_bust_player(UUID, UUID, INTEGER, TEXT) TO authenticated, service_role;
...
GRANT EXECUTE ON FUNCTION public.restore_busted_player_to_seat(UUID, UUID, INTEGER, UUID, TEXT) TO authenticated, service_role;
...
GRANT EXECUTE ON FUNCTION public.close_tournament(UUID, TEXT) TO authenticated, service_role;
...
GRANT EXECUTE ON FUNCTION public.close_tournament_table(UUID, TEXT, TEXT) TO authenticated, service_role;
...
GRANT EXECUTE ON FUNCTION public.redraw_tournament(UUID, TEXT, UUID[], INTEGER, TEXT, BOOLEAN) TO authenticated, service_role;
...
GRANT EXECUTE ON FUNCTION public.open_tournament_table(UUID, INTEGER, INTEGER) TO authenticated, service_role;

-- ============================================================================
-- Seat Assignment Module — Phase 2: Atomic confirm + assign RPC
-- ============================================================================
...
BEGIN
  ...
  IF v_seat_number IS NULL THEN
    -- Should not happen (table had capacity under the tournament lock).
  ...
  -- table_id here must be tournament_tables.id per live FK constraint.
  ...
  -- 15. Issue receipt (retry code on the unlikely UNIQUE collision).
  LOOP
...
GRANT EXECUTE ON FUNCTION public.confirm_registration_and_assign_seat(UUID, UUID, TEXT) TO authenticated;

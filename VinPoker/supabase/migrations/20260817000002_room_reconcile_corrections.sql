-- ============================================================================
-- DEALER SWING — ROOM RECONCILIATION & CORRECTION MODE (Session A: DB/RPC only)
--
-- Floor-controlled atomic correction of domino dealer<->table mismatches.
-- Example: system says A relieved B at Table 1; reality: A relieved C at
-- Table 2 ten minutes ago and system shows D at Table 2. Fixing one table at
-- a time creates duplicate active dealers and bad future scheduling; this RPC
-- corrects the whole affected room in ONE transaction with before/after
-- snapshots, validated diff, and audit.
--
-- SOURCE-ONLY: do NOT apply live without owner approval (manual-gated).
--
-- ── Preflight schema verification (read from migrations on origin/main) ────
-- dealer_attendance.current_state CHECK:
--   ('available','assigned','on_break','checked_out','pre_assigned')
--   [20260524000001 + 20260530000005]. NOTE: 'checked_out' IS valid, but the
--   no_show resolution deliberately uses the on_break hold pattern per owner
--   instruction (reversible, visible in break pool, scheduler-safe).
-- dealer_attendance.status CHECK:
--   ('scheduled','checked_in','checked_out','absent','overtime')
-- dealer_assignments.status CHECK: ('assigned','on_break','completed');
--   version auto-bumped by trg_dealer_assignments_version (BEFORE UPDATE sets
--   NEW.version := OLD.version + 1); release_reason column exists (live
--   baseline 20260611000001; written by assign_dealer_to_table v2, patch K).
-- Active-row definition = released_at IS NULL AND status IN
--   ('assigned','on_break')  [idx_one_active_per_dealer UNIQUE(attendance_id),
--   20260609000007]. No unique index exists on table_id for active rows —
--   table-level uniqueness MUST be checked procedurally here.
-- dealer_breaks: assignment_id (nullable since 20260805000000),
--   attendance_id, club_id, break_start, break_end,
--   expected_duration_minutes, reason [20260526000001].
-- audit_logs: (id, club_id, actor_id, action, entity_type, entity_id,
--   payload, created_at) [20260522000001]; frontend useAuditLogs reads
--   `payload` (live baseline fn tournament_break references `metadata` —
--   drift noted; this RPC uses `payload`).
-- Staff gate + grants copied verbatim from 20260816000000
--   set_rotation_slot_dealer: is_club_dealer_control / is_club_admin /
--   has_role(super_admin); REVOKE FROM PUBLIC, anon; GRANT TO authenticated,
--   service_role (service_role passes gate via auth.uid() IS NULL).
-- swing_config: swing_duration_minutes (default 45), break_duration_minutes
--   (default 20), UNIQUE(club_id, table_type), table_type IN
--   ('cash','tournament','vip').
--
-- ── Owner-locked rules ──────────────────────────────────────────────────────
-- * Payroll fields NEVER written: check_in_time, check_out_time,
--   overtime_minutes. Would-be OT recorded in diff.payroll_reconciliation.
-- * total_worked_minutes_today credited ONLY on release, NEVER on move;
--   GREATEST(0, ...); released_at IS NULL guard prevents double-credit.
-- * effective_at: <= 120 min in the past for dealer_control; older requires
--   admin override (club_admin/super_admin); future (> now()+1min) rejected.
--   effective_at < assigned_at of a released row → blocking conflict,
--   computed in P3 against actual rows (under locks in apply mode); with
--   admin override the per-row release timestamp clamps to assigned_at.
-- * Move (right dealer, wrong table) = UPDATE table_id in place; preserves
--   assigned_at / swing_due_at; no fake released_at (protects the R1 rest
--   anchor — see 20260814000000).
-- * Displaced dealers resolved PER DEALER (no global auto-status):
--     pool_available (default) | on_break | still_working_other_table
--     | unknown_needs_floor_check | no_show
-- * Rotation: data-level supersede only (predicted/announced slots →
--   'superseded'); planner replans next tick; zero planner code changes.
--
-- Rollback: DROP FUNCTION public.reconcile_dealer_room_state(uuid, jsonb,
--   timestamptz, text, jsonb, boolean, boolean); keep the corrections table
--   (audit data) — drop only if never used.
-- ============================================================================

BEGIN;

-- ────────────────────────────────────────────────────────────────────────────
-- [1] Correction audit table
-- ────────────────────────────────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS public.dealer_assignment_corrections (
  id                      uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  club_id                 uuid NOT NULL REFERENCES public.clubs(id),
  affected_table_ids      uuid[] NOT NULL,
  affected_attendance_ids uuid[] NOT NULL,
  affected_dealer_ids     uuid[] NOT NULL,
  effective_at            timestamptz NOT NULL,
  reason                  text NOT NULL CHECK (length(btrim(reason)) >= 5),
  created_by              uuid,            -- auth.uid(); NULL = service_role
...
CREATE INDEX IF NOT EXISTS idx_dac_club_created
  ON public.dealer_assignment_corrections (club_id, created_at DESC);

ALTER TABLE public.dealer_assignment_corrections ENABLE ROW LEVEL SECURITY;

-- Staff read-only. NO INSERT/UPDATE/DELETE policy for authenticated:
-- only the SECURITY DEFINER RPC (owner bypasses RLS) writes rows.
DROP POLICY IF EXISTS "dac_select_staff" ON public.dealer_assignment_corrections;
CREATE POLICY "dac_select_staff" ON public.dealer_assignment_corrections
  FOR SELECT USING (
...
DROP POLICY IF EXISTS "dac_service_all" ON public.dealer_assignment_corrections;
CREATE POLICY "dac_service_all" ON public.dealer_assignment_corrections
  FOR ALL USING (auth.role() = 'service_role')
  WITH CHECK (auth.role() = 'service_role');

-- ────────────────────────────────────────────────────────────────────────────
-- [2] RPC reconcile_dealer_room_state
--
-- p_corrections: jsonb array of
--   { "table_id": uuid,
--     "actual_attendance_id": uuid | null,   -- null = table is actually empty
--     "confirm_empty": bool,                 -- required true when null above
--     "expected_assignment_id": uuid | null, -- CAS echo from dry-run
--     "expected_version": int | null }       -- CAS echo from dry-run
-- p_displaced: jsonb array of
--   { "attendance_id": uuid,
--     "resolution": 'pool_available'|'on_break'|'still_working_other_table'
--                  |'unknown_needs_floor_check'|'no_show',
--     "reason": text | null }
-- ────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  FOR v_entry IN SELECT value FROM jsonb_array_elements(p_corrections) LOOP
    ...
    IF v_aid IS NOT NULL THEN
      ...
      WHERE att.id = v_aid AND att.club_id = p_club_id AND att.status = 'checked_in';
      IF NOT FOUND THEN
...
-- ────────────────────────────────────────────────────────────────────────────
-- [3] Grants — verbatim pattern from set_rotation_slot_dealer.
-- ────────────────────────────────────────────────────────────────────────────

REVOKE EXECUTE ON FUNCTION public.reconcile_dealer_room_state(uuid, jsonb, timestamptz, text, jsonb, boolean, boolean) FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION public.reconcile_dealer_room_state(uuid, jsonb, timestamptz, text, jsonb, boolean, boolean) TO authenticated, service_role;

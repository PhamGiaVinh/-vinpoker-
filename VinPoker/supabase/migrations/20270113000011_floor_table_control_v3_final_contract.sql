-- ============================================================================
-- Floor Table Control V3 — final consolidated contract (SOURCE-ONLY / RED)
-- ============================================================================
-- Depends on: 20270113000010_floor_table_control_v3_final_live_bridge.sql
--
-- This successor consolidates the intended server-contract, hardening and
-- roster-read behavior preserved unchanged in the archived 20270113000003,
-- 20270113000005 and 20270113000006 sources.  It does not rewrite legacy
-- table_id values, enable V3 writers, deploy Edge, change a feature flag, or
-- modify financial state or chip balances.
--
-- ROLLBACK (owner-gated): revoke the public V3 RPCs and restore only a proven
-- legacy constraint through a new reviewed migration. Never replay archived
-- sources or repair the migration ledger.
-- ============================================================================
...
-- exact single-column legacy constraint after the bridge has linked every
-- operational row.  Historical, completed and quarantined rows intentionally
-- remain outside this preflight and keep their legacy identity unchanged.
DO $$
DECLARE
...
BEGIN
  IF EXISTS (
    ...
    WHERE tournament_row.deleted_at IS NULL
      AND tournament_row.status NOT IN ('completed', 'cancelled')
      ...
      AND (
        ...
        OR session_row.club_id IS DISTINCT FROM tournament_row.club_id
      )
  ) THEN
    RAISE EXCEPTION USING
      ERRCODE = 'P0001',
      MESSAGE = 'floor_table_v3_legacy_assignment_preflight_failed',
      DETAIL = 'Operational tournament-table rows require exact V3 identity links before the legacy uniqueness contract can change.';
  END IF;
...
-- Consolidated hardening from archived 20270113000005.
CREATE SCHEMA IF NOT EXISTS floor_private;
REVOKE ALL ON SCHEMA floor_private FROM PUBLIC, anon, authenticated, service_role;
...
-- Consolidated roster read contract from archived 20270113000006.

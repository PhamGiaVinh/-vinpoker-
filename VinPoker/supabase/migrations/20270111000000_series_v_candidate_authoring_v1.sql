-- Series V Candidate Authoring V1 (source only).
--
-- Provides an owner-scoped path from one scheduled tournament to one
-- versioned Series V candidate. The client may select a tournament and supply
-- explicitly-labelled owner inputs, but the server always re-reads the
-- tournament and constructs the evidence manifest before approving a candidate.
-- This migration does not apply itself, deploy an Edge Function, or enable a UI flag.
...
BEGIN
  ...
  END IF;
  IF v_tournament.status IS DISTINCT FROM 'scheduled' THEN
...
BEGIN
  ...
  END IF;
  IF v_tournament.status IS DISTINCT FROM 'scheduled'
...
-- DROP FUNCTION public.series_approve_schedule_candidate_from_tournament_v1(uuid, uuid, bigint, bigint, integer, integer);
-- DROP FUNCTION public.series_preview_schedule_candidate_v1(uuid, uuid);
-- DROP FUNCTION public.series_list_schedule_candidate_sources_v1(uuid);

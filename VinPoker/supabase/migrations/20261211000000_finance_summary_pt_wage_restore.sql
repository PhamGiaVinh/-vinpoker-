-- ============================================================================================
-- REPAIR WAVE R2 (2026-07-02, owner-approved): restore PART-TIME wage cost into
-- get_club_finance_summary. SOURCE-ONLY migration: NOT applied on merge — applied only via the
-- owner-triggered "Repair wave — apply" workflow (finance-pt-wage), which snapshots before/after.
--
-- WHY: the fnb finance rewrites (20261111000006 → …000011 → …000013) each rebuilt this function
-- from the pre-PT v3 lineage and silently DROPPED the pt_pay leg added by v4 (20261028000001).
-- Verified on live 2026-07-02 (read-only pg_get_functiondef dump): the live body == …000013 and
-- contains ZERO references to dealer_pt_wage_payments → the Owner Finance Dashboard under-counts
-- payroll cost (and over-states net) by the total of part-time wage payouts.
--
-- BASE OF THIS BODY: the EXACT live dump (scripts/repair-wave/rollback file = that dump verbatim),
-- NOT any source migration — per owner instruction, source is not trusted as live truth.
-- CHANGES vs live (golden diff, marked [PT]):
--   1. empty-state cost object          + 'ptWagePaid',0
--   2. new CTE pt_pay                   (dealer_pt_wage_payments, cash basis by paid_at, non-void)
--   3. cost.payrollNet                  + pt_pay sum; new cost.ptWagePaid (itemized, already inside
--                                         payrollNet — client must NOT add it again)
--   4. net                              − pt_pay sum
--   5. trend.cost_v + month spine       + pt_pay
--   6. perClub.cost_v                   + pt_pay
-- Nothing else changes: staking/rake/service-fee/F&B legs byte-identical to live.
--
-- ROLLBACK: supabase/migrations/_repair_finance_pt_wage_rollback.sql (exact pre-repair live body).
-- ============================================================================================
  ...
  if v_all_ids is null or array_length(v_all_ids, 1) is null then
    return jsonb_build_object(
      ...
      'cost', jsonb_build_object(
        'payrollNet',0,'payrollGross',0,'adjustments',0,'ptWagePaid',0,'fnbCogs',0,'compCogs',0),  -- [C] +compCogs [PT] +ptWagePaid

-- Fix verify_swing_queries: perform_swing returns JSONB, not composite
...
BEGIN
  ...
  SELECT auto_swing_enabled INTO v_cnt FROM club_settings WHERE club_id = p_club_id;
  v_results := array_append(v_results, jsonb_build_object('check', 'Auto swing enabled', 'pass', v_cnt = true, 'value', v_cnt));

  -- B4: RPC validation (JSONB return)
  ...
  -- B15: state column
  SELECT column_name INTO v_col FROM information_schema.columns WHERE table_name = 'dealer_assignments' AND column_name = 'state';
  v_results := array_append(v_results, jsonb_build_object('check', 'state column exists', 'pass', v_col IS NOT NULL, 'value', v_col));

-- Fix seed_swing_test_data() and verify_swing_queries() from 20260701000001
-- Changes: dealer_assignments/dealer_attendance have no club_id column directly
...
BEGIN
  ...
  -- A5. Check-in dealers (dealer_attendance has no club_id column)
  WITH inserted AS (
...
BEGIN
  ...
  v_results := array_append(v_results, jsonb_build_object('check', 'Pool snapshot exists', 'pass', v_pool IS NOT NULL, 'value', jsonb_build_object('available', v_pool.available, 'weighted', v_pool.weighted_pool)));

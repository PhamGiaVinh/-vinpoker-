-- Series V Candidate Authoring source-state compatibility.
--
-- The tournament state machine does not support `scheduled`; events planned for
-- a future start are created as active, upcoming, or registering. This migration
-- replaces the three authoring RPC bodies so their eligibility gate matches that
-- state machine while retaining the existing owner, source-evidence, and
-- re-read-before-approval boundaries.
...
BEGIN
  ...
  FROM (
    ...
    WHERE t.club_id = p_club_id
      ...
      AND t.status IN ('active', 'upcoming', 'registering')
...
BEGIN
  ...
  END IF;
  IF NOT COALESCE(v_tournament.status = ANY (ARRAY['active'::text, 'upcoming'::text, 'registering'::text]), false) THEN
...
BEGIN
  ...
  END IF;
  IF NOT COALESCE(v_tournament.status = ANY (ARRAY['active'::text, 'upcoming'::text, 'registering'::text]), false)
...
-- 20270111000000_series_v_candidate_authoring_v1.sql in a new migration.

-- ═══════════════════════════════════════════════════════════════════════════════
-- PAYROLL P3 — CROSS-MONTH OVERLAP ATTRIBUTION — calculate_dealer_payroll
--
...
-- (docs/payroll/P3_CROSS_MONTH_GOLDEN_DIFF_PLAN.md). NO `supabase db push`, NO deploy_db.
--
-- BUG (finding #4): the attendance loop filtered `check_in_time::DATE BETWEEN start AND
--   end` and counted the WHOLE shift, so an overnight shift crossing a month boundary
--   was attributed entirely to the check-in month; the next month got nothing.
--   Live 2026-06-16: 2 real shifts (05-31 22:xx → 06-01) put ~12.8h of June work in May.
--
-- FIX (owner policy 2026-06-16): allocate each shift to a payroll period by its ACTUAL
--   overlap with the Vietnam-local half-open period [period_start, period_end):
--     overlap_ratio = overlap_minutes / total_effective_shift_minutes
--   * regular & OT hours credited to the period = full-shift regular/OT × overlap_ratio
--   * base-salary SHIFT CREDIT = overlap_ratio (a same-month shift = 1; a cross-month
--     shift sums to exactly 1 across its two months; OT never pushes credit above 1)
--   The filter widens from "check-in in period" to "shift overlaps period". Open shifts
--   keep the P2 effective checkout (check_in + standard hours) — now() is NOT used here.
--
-- CONSERVATION: a shift's minutes/regular/OT/credit summed across its two adjacent
--   periods equal the original single-shift totals. Same-month shifts: overlap_ratio = 1
--   → identical to the live P2 body (modulo rounding).
--
-- DOES NOT CHANGE: P0 BHXH base (still v_base_salary_vnd), P1 TIPS=0, P2 open-shift
--   rule, PIT formula (only its input hours/base move), P4a/P4b.
--
-- ROLLBACK: re-apply migration 20260906000000_payroll_p2_open_shift_standard.sql
--   (the live P2 body).
-- ═══════════════════════════════════════════════════════════════════════════════

...
BEGIN
  ...
  -- Serialize session open/close with assignment writers using the physical
  -- table lock. Exactly one active session binds; none preserves legacy mode.
  PERFORM 1
  FROM game_tables
  WHERE id = v_table_id
  ...
  WHERE ts.game_table_id = v_table_id
  ...
  IF v_active_session_count > 1 THEN
    RETURN jsonb_build_object(
      'status', 'error',
      'error',  'TABLE_SESSION_AMBIGUOUS',
      'detail', v_table_id
    );
  END IF;

  IF v_active_session_count = 0 AND EXISTS (
    SELECT 1
    FROM table_sessions ts
    WHERE ts.game_table_id = v_table_id
      AND ts.club_id = v_club_id
  ...
  IF v_active_session_count = 1 THEN
    ...
    IF NOT FOUND THEN
      RETURN jsonb_build_object(
        'status', 'error',
        'error',  'TABLE_SESSION_CHANGED',
        'detail', v_table_id
      );
    END IF;

    IF v_old_table_session_id IS NOT NULL
       AND v_old_table_session_id IS DISTINCT FROM v_table_session_id THEN
      RETURN jsonb_build_object(
        'status', 'error',
        'error',  'TABLE_SESSION_STALE',
        'detail', v_old_table_session_id
      );
    END IF;

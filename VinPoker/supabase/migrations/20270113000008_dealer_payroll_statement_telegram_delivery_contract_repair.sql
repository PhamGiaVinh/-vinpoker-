-- Forward-only repair for immutable payroll statement Telegram delivery.
--
-- CRITICAL / RED. The historical 20270113000004 migration may appear in the
-- remote ledger while its catalog objects are absent. Never replay it. This
-- migration accepts only a fully absent delivery contract or an already
-- complete contract; any partial catalog state fails closed.
...
declare
  v_any boolean;
  v_complete boolean;
  v_acl_complete boolean;
begin
  v_any :=
    to_regclass('public.dealer_payroll_statement_delivery_rollout') is not null
    or to_regclass('public.dealer_payroll_delivery_operations') is not null
    or to_regclass('public.dealer_payroll_delivery_targets') is not null
    or to_regclass('public.dealer_payroll_delivery_operations_club_period_idx') is not null
    or to_regclass('public.dealer_payroll_delivery_operations_active_club_period_uq') is not null
    or to_regclass('public.dealer_payroll_delivery_targets_active_statement_channel_uq') is not null
    or to_regclass('public.dealer_payroll_delivery_targets_operation_state_idx') is not null
    or to_regclass('public.dealer_payroll_delivery_attempts_target_uq') is not null
    or exists (
      select 1 from pg_attribute
      where attrelid = to_regclass('public.dealer_payroll_delivery_attempts')
        and attname in ('operation_id', 'target_id') and not attisdropped
    )
    or to_regprocedure('public._dealer_payroll_statement_delivery_allowed(uuid)') is not null
    or to_regprocedure('public._assert_dealer_payroll_statement_delivery_rollout(uuid)') is not null
    or to_regprocedure('public.get_dealer_payroll_statement_delivery_rollout(uuid)') is not null
    or to_regprocedure('public._refresh_dealer_payroll_delivery_operation(uuid)') is not null
    or to_regprocedure('public.create_dealer_payroll_statement_delivery_operation(uuid,uuid,uuid)') is not null
    or to_regprocedure('public.get_dealer_payroll_statement_delivery_operation(uuid)') is not null
    or to_regprocedure('public.claim_dealer_payroll_statement_delivery_target(uuid)') is not null
    or to_regprocedure('public.complete_dealer_payroll_statement_delivery_target(uuid,uuid,text,text)') is not null
    or to_regprocedure('public.fail_dealer_payroll_statement_delivery_target(uuid,uuid,text,text,integer)') is not null;

  v_complete :=
    to_regclass('public.dealer_payroll_statement_delivery_rollout') is not null
    and to_regclass('public.dealer_payroll_delivery_operations') is not null
    and to_regclass('public.dealer_payroll_delivery_targets') is not null
    and to_regclass('public.dealer_payroll_delivery_operations_club_period_idx') is not null
    and to_regclass('public.dealer_payroll_delivery_operations_active_club_period_uq') is not null
    and to_regclass('public.dealer_payroll_delivery_targets_active_statement_channel_uq') is not null
    and to_regclass('public.dealer_payroll_delivery_targets_operation_state_idx') is not null
    and to_regclass('public.dealer_payroll_delivery_attempts_target_uq') is not null
    and 2 = (
      select count(*) from pg_attribute
      where attrelid = to_regclass('public.dealer_payroll_delivery_attempts')
        and attname in ('operation_id', 'target_id') and not attisdropped
    )
    and to_regprocedure('public._dealer_payroll_statement_delivery_allowed(uuid)') is not null
    and to_regprocedure('public._assert_dealer_payroll_statement_delivery_rollout(uuid)') is not null
    and to_regprocedure('public.get_dealer_payroll_statement_delivery_rollout(uuid)') is not null
    and to_regprocedure('public._refresh_dealer_payroll_delivery_operation(uuid)') is not null
    and to_regprocedure('public.create_dealer_payroll_statement_delivery_operation(uuid,uuid,uuid)') is not null
    and to_regprocedure('public.get_dealer_payroll_statement_delivery_operation(uuid)') is not null
    and to_regprocedure('public.claim_dealer_payroll_statement_delivery_target(uuid)') is not null
    and to_regprocedure('public.complete_dealer_payroll_statement_delivery_target(uuid,uuid,text,text)') is not null
    and to_regprocedure('public.fail_dealer_payroll_statement_delivery_target(uuid,uuid,text,text,integer)') is not null;

  if v_any and not v_complete then
    raise exception 'PAYROLL_DELIVERY_PARTIAL_DRIFT' using errcode = 'P0001';
  end if;

  if v_complete then
    select
      coalesce(has_function_privilege('authenticated', 'public.get_dealer_payroll_statement_delivery_rollout(uuid)', 'EXECUTE'), false)
      and coalesce(has_function_privilege('authenticated', 'public.create_dealer_payroll_statement_delivery_operation(uuid,uuid,uuid)', 'EXECUTE'), false)
      and coalesce(has_function_privilege('authenticated', 'public.get_dealer_payroll_statement_delivery_operation(uuid)', 'EXECUTE'), false)
      and not coalesce(has_function_privilege('anon', 'public.get_dealer_payroll_statement_delivery_rollout(uuid)', 'EXECUTE'), true)
      and not coalesce(has_function_privilege('anon', 'public.create_dealer_payroll_statement_delivery_operation(uuid,uuid,uuid)', 'EXECUTE'), true)
      and not coalesce(has_function_privilege('anon', 'public.get_dealer_payroll_statement_delivery_operation(uuid)', 'EXECUTE'), true)
      and coalesce(has_function_privilege('service_role', 'public._assert_dealer_payroll_statement_delivery_rollout(uuid)', 'EXECUTE'), false)
      and coalesce(has_function_privilege('service_role', 'public.claim_dealer_payroll_statement_delivery_target(uuid)', 'EXECUTE'), false)
      and coalesce(has_function_privilege('service_role', 'public.complete_dealer_payroll_statement_delivery_target(uuid,uuid,text,text)', 'EXECUTE'), false)
      and coalesce(has_function_privilege('service_role', 'public.fail_dealer_payroll_statement_delivery_target(uuid,uuid,text,text,integer)', 'EXECUTE'), false)
      and not coalesce(has_function_privilege('authenticated', 'public.claim_dealer_payroll_statement_delivery_target(uuid)', 'EXECUTE'), true)
      and not coalesce(has_function_privilege('authenticated', 'public.complete_dealer_payroll_statement_delivery_target(uuid,uuid,text,text)', 'EXECUTE'), true)
      and not coalesce(has_function_privilege('authenticated', 'public.fail_dealer_payroll_statement_delivery_target(uuid,uuid,text,text,integer)', 'EXECUTE'), true)
      and not coalesce(has_table_privilege('authenticated', 'public.dealer_payroll_statement_delivery_rollout', 'SELECT,INSERT,UPDATE,DELETE'), true)
      and not coalesce(has_table_privilege('authenticated', 'public.dealer_payroll_delivery_operations', 'SELECT,INSERT,UPDATE,DELETE'), true)
      and not coalesce(has_table_privilege('authenticated', 'public.dealer_payroll_delivery_targets', 'SELECT,INSERT,UPDATE,DELETE'), true)
      and not coalesce(has_table_privilege('anon', 'public.dealer_payroll_statement_delivery_rollout', 'SELECT,INSERT,UPDATE,DELETE'), true)
      and not coalesce(has_table_privilege('anon', 'public.dealer_payroll_delivery_operations', 'SELECT,INSERT,UPDATE,DELETE'), true)
      and not coalesce(has_table_privilege('anon', 'public.dealer_payroll_delivery_targets', 'SELECT,INSERT,UPDATE,DELETE'), true)
      and coalesce(has_table_privilege('service_role', 'public.dealer_payroll_statement_delivery_rollout', 'SELECT,UPDATE'), false)
      and coalesce(has_table_privilege('service_role', 'public.dealer_payroll_delivery_operations', 'SELECT,INSERT,UPDATE'), false)
      and coalesce(has_table_privilege('service_role', 'public.dealer_payroll_delivery_targets', 'SELECT,INSERT,UPDATE'), false)
    into v_acl_complete;

    if not coalesce(v_acl_complete, false)
       or exists (
         select 1
       ...
       or exists (
         ...
         where c.oid in (
           'public.dealer_payroll_statement_delivery_rollout'::regclass,
           'public.dealer_payroll_delivery_operations'::regclass,
           'public.dealer_payroll_delivery_targets'::regclass
...
grant execute on function public._assert_dealer_payroll_statement_delivery_rollout(uuid) to service_role;
...
do $repair_postcheck$
begin
  if to_regclass('public.dealer_payroll_statement_delivery_rollout') is null
     ...
     or to_regprocedure('public._dealer_payroll_statement_delivery_allowed(uuid)') is null
     or to_regprocedure('public._assert_dealer_payroll_statement_delivery_rollout(uuid)') is null
     or to_regprocedure('public.get_dealer_payroll_statement_delivery_rollout(uuid)') is null
     or to_regprocedure('public._refresh_dealer_payroll_delivery_operation(uuid)') is null
     or to_regprocedure('public.create_dealer_payroll_statement_delivery_operation(uuid,uuid,uuid)') is null
     or to_regprocedure('public.get_dealer_payroll_statement_delivery_operation(uuid)') is null
     or to_regprocedure('public.claim_dealer_payroll_statement_delivery_target(uuid)') is null
     or to_regprocedure('public.complete_dealer_payroll_statement_delivery_target(uuid,uuid,text,text)') is null
     or to_regprocedure('public.fail_dealer_payroll_statement_delivery_target(uuid,uuid,text,text,integer)') is null
     or (select count(*) from public.dealer_payroll_statement_delivery_rollout where id = true) <> 1
     or exists (
       select 1 from pg_class c
       where c.oid in (
         'public.dealer_payroll_statement_delivery_rollout'::regclass,
    ...
    raise exception 'PAYROLL_DELIVERY_REPAIR_POSTCHECK_FAILED' using errcode = 'P0001';
  end if;
end;
$repair_postcheck$;

-- ============================================================================
-- ROOM RECONCILE 33E FIX: mutual-swap moves vs idx_unique_active_assignment
--
-- Bug (found in the 33D production smoke, 2026-06-13): phase [5.2] performed
-- sequential in-place table_id moves. In a swap cycle (dealer A recorded at
-- T1 actually deals T2 and vice versa) the first UPDATE transiently puts two
-- status='assigned' rows on one table, and the LIVE partial unique index
--   idx_unique_active_assignment ON dealer_assignments(table_id)
--   WHERE status='assigned'           (20260531000001, confirmed in pg_indexes)
-- rejects it: duplicate key -> unique_violation handler -> outcome
-- 'race_lost', full rollback. SAFE failure, but the swap path could never
-- succeed; the original design comment wrongly assumed no unique index on
-- table_id existed. One-sided corrections (release -> assign) were unaffected
-- and passed the 33D smoke live (correction bc2341b3-...).
--
-- Fix: phase [5.2] becomes two-pass PARK & PLACE (see inline comment).
-- Everything else is byte-identical to 20260818000002 (which itself only
-- fixed the att.club_id reference of 20260817000002). One-sided release /
-- assign / supersede / audit / CAS / permission behavior unchanged.
--
-- SOURCE-ONLY: do NOT apply live without owner approval (manual-gated).
-- Rollback: re-run the function block of 20260818000002 (restores swap-broken
-- but otherwise identical v2).
-- ============================================================================
...
DECLARE
  ...
  v_released        int := 0;
  v_moved           int := 0;
  -- 33E: assignment ids parked (status assigned->on_break) during pass 1 of
  -- the two-pass move, so pass 2 knows which rows to restore to 'assigned'.
  v_parked          uuid[] := '{}'::uuid[];
  v_assigned        int := 0;
  v_superseded      int := 0;
  v_action          text;
...
BEGIN
  ...
  -- 33E FIX (this migration): two-pass PARK & PLACE. The live partial unique
  -- index idx_unique_active_assignment (table_id) WHERE status='assigned'
  -- rejects the transient duplicate that sequential in-place moves create in
  -- a swap cycle (both rows briefly point at one table — 33D smoke finding:
  -- unique_violation surfaced as race_lost, so a mutual swap could never
  -- apply). Pass 1 parks every moving row that is status='assigned' as
  -- 'on_break': the row leaves the table_id index (predicate
  -- status='assigned') while STAYING inside idx_one_active_per_dealer
  -- (released_at IS NULL AND status IN ('assigned','on_break')) — per-dealer
  -- uniqueness holds throughout. Rows are already locked (P1) and the parked
  -- state is invisible outside this transaction. Pass 2 places each row on
  -- its target table and restores parked rows to 'assigned' (rows that were
  -- genuinely on_break keep their status, as before). Any failure raises →
  -- the whole transaction rolls back. released_at / worked minutes /
  -- attendance remain untouched (moves never release).
  ...
  FOR v_entry IN SELECT value FROM jsonb_array_elements(v_plan)
    ...
    IF v_row.status = 'assigned' THEN
      UPDATE dealer_assignments SET
        ...
        version    = version + 1,
        updated_at = v_now
      WHERE id = v_row.id AND released_at IS NULL AND status = 'assigned';
      IF NOT FOUND THEN
        RAISE EXCEPTION 'reconcile: move park failed on assignment %', v_row.id;
      END IF;
      v_parked := v_parked || v_row.id;
    END IF;
  END LOOP;
  ...
  FOR v_entry IN SELECT value FROM jsonb_array_elements(v_plan)
    ...
    UPDATE dealer_assignments SET
      ...
      pre_assigned_at            = NULL,
      planned_relief_at          = NULL,
      status                     = CASE WHEN id = ANY(v_parked) THEN 'assigned' ELSE status END,
      version                    = version + 1,
      updated_at                 = v_now
    WHERE id = v_row.id AND released_at IS NULL;
    IF NOT FOUND THEN

...
-- 2b. RLS helpers (SECURITY DEFINER) — boolean event/snapshot↔club membership, bypassing the inner
--     table's RLS so a legitimate owner's WITH CHECK passes (an inline nested EXISTS would evaluate false).
CREATE OR REPLACE FUNCTION public.series_event_in_club(_event uuid, _club uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$ SELECT EXISTS (SELECT 1 FROM public.tournaments t WHERE t.id = _event AND t.club_id = _club) $$;
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$ SELECT EXISTS (SELECT 1 FROM public.series_forecast_snapshots s WHERE s.id = _snapshot AND s.club_id = _club) $$;
...
BEGIN
  ...
  BEGIN
    ...
    VALUES (v_owned, v_ev_o, v_snap, 'T-7', 'add_overlay')  -- use the plpgsql var (reading _dryrun_ids under role=authenticated = permission denied)
  ...
  BEGIN
    ...
    UPDATE public.series_decision_logs
     ...
     WHERE id = v_dec;  -- use the plpgsql var (reading _dryrun_ids under role=authenticated = permission denied)

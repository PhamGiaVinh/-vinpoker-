-- Phase 5 PR #1 - BUG #5: Add p_rest_deficit_minutes to perform_swing RPC
-- BUG #5: Rest deficit was only calculated for pre-assign path, NOT for direct swing path
-- Fix: Add 8th parameter p_rest_deficit_minutes to both overloads
-- When passed, swing_due_at is delayed by this many minutes (ceil((breakEnd - now) / 60000))

-- STEP 1: Drop old 7-param overloads (no p_rest_deficit_minutes)
-- The CREATE OR REPLACE in next steps adds NEW overloads with 8 params
...
-- STEP 2: Create new 8-param wrapper overload
...
-- STEP 3: Create new 8-param core overload
CREATE OR REPLACE FUNCTION public.perform_swing(
  p_assignment_id uuid,
  p_version integer,
  p_next_attendance_id uuid,
...
BEGIN
  ...
  -- BUG #5 FIX: Apply rest deficit to compensate for insufficient rest
...
COMMENT ON FUNCTION public.perform_swing(uuid, integer, boolean, integer, integer, integer, uuid, integer) IS
  'Phase 5 PR #1 BUG #5: Wrapper overload - delegates to 8-param core. Added p_rest_deficit_minutes parameter.';

COMMENT ON FUNCTION public.perform_swing(uuid, integer, uuid, boolean, integer, integer, timestamptz, integer) IS
  'Phase 5 PR #1 BUG #5: Core 7-param + new p_rest_deficit_minutes parameter. Delays swing_due_at by rest deficit minutes.';

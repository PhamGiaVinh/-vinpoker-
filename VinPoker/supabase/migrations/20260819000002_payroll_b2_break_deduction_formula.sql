-- ═══════════════════════════════════════════════════════════════════════════════
-- PAYROLL B2-PR3: BREAK DEDUCTION FORMULA — calculate_dealer_payroll
--
-- Owner policy (docs/payroll/B2_BREAK_PAY_MODE_DECISION.md, Q1-Q3 all decided):
--   paid_break (DEFAULT)      no deduction — formula behavior byte-equivalent to before
--   unpaid_break_with_grace   clamp each break to [check_in, check_out], merge
--                             overlapping intervals, deduct minutes beyond
--                             grace_minutes PER MERGED INTERVAL
--   unpaid_break_full         clamp + merge, deduct all valid break minutes
-- Q3: deduction is applied to shift hours BEFORE the regular/OT split (no fake OT).
-- FT semantics: base salary stays shift-prorated; deduction affects OT hours only.
-- PT semantics: hourly pay reduced directly.
--
-- Three anchored insertions into the canonical body (md5 0ffc563d65f12790a17b4673c82a3dce):
--   1. DECLARE: v_break_pay_mode / v_break_grace_minutes / v_break_deduct_hours
--   2. policy lookup from shift_break_policies (club_id, shift_type='default';
--      missing row => paid_break/35)
--   3. per-shift deduction block (skipped entirely in paid_break mode)
--
-- Verified live (2026-06-13, controlled apply; live md5 59c28609f586c5f80b83a2b1ea8daf97):
--   - same-instant old-vs-new across ALL 39 active dealers with paid_break: 0 mismatches
--   - club 11 with_grace(35): 0 deductions — independently proven correct
--     (max merged clamped interval = 23.9 min, none exceed grace)
--   - club 11 unpaid_break_full: 8/10 dealers deducted, total 421 min — EXACT match
--     with the independent B2-PR1 audit clamp numbers (296 FT + 125 PT);
--     FT deltas hit OT only; FT in-regular deductions = 0 VND (by design);
--     PT reduced hourly; insurance/PIT recomputed correctly
--   - club 22 June saved-path RPC output byte-identical before/after
--   - stored dealer_payroll md5 unchanged (4a786968725b8879272ee701e576579b)
--   - policies reverted to paid_break everywhere after testing
--
-- ROLLBACK: re-apply docs/emergency_rollbacks/PRE_B2PR3_calculate_dealer_payroll_live_snapshot_20260613.sql
-- ═══════════════════════════════════════════════════════════════════════════════

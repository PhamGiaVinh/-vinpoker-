...
Deno.serve(async (req) => {
  ...
  if (!tournament_id) {
    return new Response(JSON.stringify({ error: 'Missing tournament_id' }), { status: 400, headers: { 'Content-Type': 'application/json' } })
  }

  try {
    const result = await supabase.rpc('get_tournament_leaderboard', { p_tournament_id: tournament_id })

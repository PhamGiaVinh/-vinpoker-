-- Dealer PT global continuous accrual v2.
--
-- SUPERSEDES WITHOUT REPLAY:
--   20270105000002_dealer_pt_wage_global_continuous_accrual.sql
--   20270105000003_dealer_pt_wage_rate_history.sql
--
-- Those historical files remain immutable and must never be applied. This
-- single forward migration requires only the live 20270105000001 baseline.
-- It creates the global policy, effective-dated PT rate history, immutable
-- payout snapshots, readiness guard, and authenticated mutation grant in one
-- transaction. The global policy stays disabled by default and this migration
-- never enables a club policy, pays wages, or rewrites historical receipts.
--
...
-- The rate-history and replacement balance/payment contract must exist before
-- the global mutation is granted. One transaction makes this atomic to callers.

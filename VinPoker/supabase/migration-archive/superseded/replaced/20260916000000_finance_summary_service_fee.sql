-- get_club_finance_summary v3 — add SERVICE FEE (phí dịch vụ) as a SEPARATE revenue stream.
...
-- push`, NO deploy_db, NO schema_migrations edit. Supersedes the 20260905000000 body (CREATE OR
-- REPLACE, same signature). Read-only / STABLE / zero writes. Never recomputes payroll.
--
-- WHAT CHANGED vs v2 (20260905000000), reporting only — ADDITIVE:
--   * New per-tour SERVICE FEE: tournaments.service_fee_amount (a SECOND configured per-entry charge,
--     separate from rake_amount; player price = buy_in + rake_amount + service_fee_amount). It is
--     charged on EVERY paying confirmed entry (free-rake waives the RAKE only, never the service fee).
--   * serviceFee (= service_fee_amount × paying confirmed entries) is a NEW revenue stream, summed into
--     revenue.total and net, kept strictly SEPARATE from rake and from staking fees.
--   * rakeActual now means RAKE ONLY: rake_actual = GREATEST(0, total_pay − buy_in − service_fee_amount).
--     Because the registration writers fold both rake and service fee into total_pay, subtracting the
--     per-entry service fee keeps the rake reconciliation honest (rakeActual stays comparable to the
--     configured rake; rakeVariance = rakeActual − rake is unchanged in meaning).
--   * For EVERY existing tour service_fee_amount = 0, so serviceFee = 0 and rake_actual is byte-identical
--     to v2 — a true no-op until an owner sets a service fee. (Golden-diff: identical except the new
--     serviceFee:0 revenue field.)
--
-- Revenue streams (club-attributable), kept strictly separate:
--   TOURNAMENT  -> rake (configured)            [tournaments.rake_amount × paying entries]
--               -> serviceFee (configured)      [tournaments.service_fee_amount × paying entries]  <-- NEW
--   STAKING     -> stakingFixed + stakingPercent + stakingArchive ; payoutFees
--
-- ROLLBACK: CREATE OR REPLACE back to the 20260905000000 body (drops serviceFee, restores rakeActual =
--   total_pay − buy_in). The useClubFinanceSummary hook treats missing fields as 0, so rollback is safe.
...
begin
  ...
  -- Total/trend/perClub revenue uses CONFIGURED rake + service fee.
  rev_all as (
  ...
  select jsonb_build_object(
    'revenue', jsonb_build_object(
      ...
      'serviceFee',     (select coalesce(sum(cfg_service),0) from rake_cfg),  -- NEW: configured service fee × paying entries

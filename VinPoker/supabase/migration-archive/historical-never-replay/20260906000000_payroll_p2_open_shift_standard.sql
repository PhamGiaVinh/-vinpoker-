-- ═══════════════════════════════════════════════════════════════════════════════
-- PAYROLL P2 — OPEN-SHIFT PHANTOM OT FIX — calculate_dealer_payroll
--
...
-- (docs/payroll/P2_OPEN_SHIFT_GOLDEN_DIFF_PLAN.md). NO `supabase db push`, NO deploy_db.
--
-- BUG (finding #2): a shift with NO check-out (still open) computed its duration as
--   `COALESCE(check_out_time, now()) - check_in_time`, capped at 24h. A dealer who
--   forgot to check out therefore accrued up to ~16h of PHANTOM overtime that grows
--   with wall-clock time (FT: regular = LEAST(hours, standard); OT = hours − standard).
--   Live data 2026-06-16: ALL 27 open June shifts produced phantom OT this way.
--
-- FIX (this patch, ONE concern only; owner policy 2026-06-16 "tính 1 ca chuẩn"):
--   when `check_out_time IS NULL`, assume the shift lasted exactly the configured
--   standard shift length (`dealers.standard_hours_per_shift`, default 8) instead of
--   extrapolating to now(). Result: an open shift = exactly 1 standard shift →
--   regular = standard, OT = 0. Closed shifts are byte-identical (COALESCE still
--   returns the real check_out_time). Builds on the P0-applied body.
--
-- EXACTLY ONE expression changes vs the live P0 body: the open-shift fallback inside
--   the attendance loop (`shift_hours_raw`, now() → check_in + standard hours).
--   Everything else — BHXH base (P0), break deduction (B2), TIPS=0 (P1), PIT,
--   cross-month attribution (P3, untouched) — is unchanged.
--
-- EXPLICITLY OUT OF SCOPE (separate patches): cross-month overnight attribution (P3),
--   BHTN cap / Insurance Participation Layer (P4b), PT PIT (P4).
--
-- ROLLBACK: re-apply migration 20260829000000_payroll_p0_bhxh_base_contract_salary.sql
--   (the live P0 body) — restores `COALESCE(check_out_time, now())`.
-- ═══════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  FOR v_shift_record IN
    SELECT
      ...
      -- P2: open shift (no check-out) => assume exactly one standard shift, NOT now()
      -- (kills phantom OT). Closed shifts keep their real check_out_time unchanged.
      EXTRACT(EPOCH FROM (COALESCE(da.check_out_time, da.check_in_time + (v_dealer.standard_hours_per_shift * interval '1 hour')) - da.check_in_time)) / 3600 AS shift_hours_raw,

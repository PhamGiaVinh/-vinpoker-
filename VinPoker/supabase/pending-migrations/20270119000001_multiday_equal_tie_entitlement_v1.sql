...
-- Occupied rank amounts are passed once each (e.g. ranks 46/47 = [M,0]).
...
BEGIN
  ...
  END IF;
  -- Until the owner chooses the indivisible-VND policy, no remainder is
  -- silently assigned to a player or manufactured as a paid obligation.
  IF pg_catalog.mod(v_total, p_tied_player_count::numeric) <> 0 THEN
    RAISE EXCEPTION 'multiday_tie_remainder_policy_required' USING ERRCODE = '22023';
  END IF;
  v_share := v_total / p_tied_player_count::numeric;

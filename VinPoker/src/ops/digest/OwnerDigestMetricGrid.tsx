...
export function OwnerDigestMetricGrid({ report }: { report: OwnerDailyDigestReport }) {
  const operations: MetricProps[] = [
    { label: "Đăng ký", value: formatCount(report.registrations), icon: TicketCheck },
  ...
  const money: MetricProps[] = [
    { label: "Rake giữ lại", value: formatMoney(report.rakeAmount), icon: Coins, tone: "positive" },
    { label: "Doanh thu F&B", value: formatMoney(report.fnbAmount), icon: UtensilsCrossed, tone: "positive" },
    { label: "Payout đang chờ", value: formatMoney(report.outstandingPayoutAmount), icon: WalletCards, tone: "warning" },
    { label: "Lương tạm tính", value: formatMoney(report.provisionalPayrollAmount), icon: ReceiptText, tone: "warning" },
  ];

  return (
    <div className="grid gap-4 xl:grid-cols-[0.86fr_1.14fr]">
      <MetricSection title="Nhịp vận hành" description="Lượng người và nguồn lực trong ngày" metrics={operations} />
      <MetricSection title="Dòng tiền vận hành" description="Giá trị lấy nguyên từ Digest đã tạo" metrics={money} />
    </div>
  );
}
...
function Metric({ label, value, icon: Icon, tone = "neutral" }: MetricProps) {
  ...
  return (
    <div className="flex min-h-16 items-center gap-3 py-3">
      ...
      <dd className={`shrink-0 text-right font-mono text-base font-semibold tabular-nums sm:text-lg ${toneClass}`}>{value}</dd>
    </div>
  );
}

...
import { buildOpsIntelligenceOverviewV1, gapLabel, type IntelligenceTabV1, type OverviewSourceV1 } from "./opsIntelligenceOverviewV1";
...
export function OpsIntelligenceOverviewV1({ clubId, scope, context, contextReason, navigate, receipts, onReceiptsChange }: {
  ...
  const detail = [...model.metrics, ...model.sources].find((row) => row.id === detailId) ?? null;
  useEffect(() => {
  ...
  const refresh = () => void Promise.allSettled([pulse.refetch(), operations.refetch(), registration.refetch(), sepay.refetch()]);
  ...
  return <section data-testid="ops-intelligence-overview-v1" className="space-y-3 text-[13px] text-[#c7d4d0]">
    ...
    <div className="grid grid-cols-12 gap-3">
      <section className="col-span-4 min-w-0 border border-white/10 bg-[#050b0d]"><h2 className="border-b border-white/10 p-3 text-sm font-semibold text-white"><CircleAlert className="mr-2 inline h-4 w-4 text-amber-200" />Việc cần xử lý <span className="text-[#a1b4ad]">({model.actions.length})</span></h2><div className="max-h-[430px] overflow-y-auto divide-y divide-white/10">{model.actions.map((action) => <div key={action.id} className="p-3"><p className={action.severity === "critical" ? "text-rose-200" : "text-amber-100"}>{action.title}</p><p className="mt-1 break-words text-xs text-[#a1b4ad]">{action.source} · {action.severity.toUpperCase()}</p><button type="button" onClick={() => navigate(action.scope, action.tab)} className="mt-2 min-h-8 text-cyan-200 underline focus-visible:outline">{action.tab === "overview" ? "Xem phạm vi" : action.tab === "live" ? "Mở Live Ops" : "Mở Data Health"}<ArrowUpRight className="ml-1 inline h-3.5 w-3.5" /></button></div>)}{!model.actions.length && <p className="p-3">Không có việc cần xử lý từ các nguồn đã đọc.</p>}</div></section>
      <section className="col-span-8 min-w-0 border border-white/10 bg-[#050b0d]"><h2 className="border-b border-white/10 p-3 text-sm font-semibold text-white"><CalendarDays className="mr-2 inline h-4 w-4 text-cyan-200" />Lịch hôm nay / sắp tới</h2>
    ...
    <section className="border border-white/10 bg-[#050b0d]"><div className="flex items-center justify-between border-b border-white/10 p-3"><h2 className="text-sm font-semibold text-white">Mức sẵn sàng dữ liệu</h2><div className="flex gap-4">{([['quant', 'Mở Quant'], ['live', 'Mở Live Ops'], ['health', 'Mở Data Health']] as const).map(([tab, label]) => <button key={tab} type="button" className="text-cyan-200 underline focus-visible:outline" onClick={() => navigate(scope, tab)}>{label}</button>)}</div></div><div className="grid grid-cols-5 divide-x divide-white/10">{model.sources.map((source) => <button key={source.id} type="button" onClick={() => setDetailId(source.id)} className="min-w-0 p-3 text-left hover:bg-white/5 focus-visible:outline"><span className="block">{source.label}</span><span className={`mt-1 block text-xs ${source.availability === "exact" ? "text-emerald-200" : "text-amber-200"}`}>{source.availability.toUpperCase()}</span></button>)}</div></section>
    <SourceSheet source={detail} receipt={detail ? receipts[`overview:${detail.id}`] : undefined} scope={scope} onClose={() => setDetailId(null)} onHealth={() => { setDetailId(null); navigate(scope, "health"); }} />
  </section>;
}

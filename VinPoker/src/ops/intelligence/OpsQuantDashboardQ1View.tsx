...
function PressureBadge({ status }: { status: QuantPressureStatus }) { const tone = status === "PRESSURE" ? "border-rose-300/25 text-rose-200" : status === "WATCH" ? "border-amber-300/25 text-amber-200" : status === "ON_TRACK" ? "border-emerald-300/25 text-emerald-200" : status === "PLANNING_SCENARIO" ? "border-cyan-300/25 text-cyan-200" : "border-white/10 text-[#71837d]"; return <span className={`inline-flex border px-1.5 py-0.5 font-mono text-[8px] ${tone}`}>{status.replaceAll("_", " ")}</span>; }

...
function AssumptionInput({ label, value, positive = false, semanticError = null, onChange }: { label: string; value: string; positive?: boolean; semanticError?: string | null; onChange: (raw: string) => void }) {
  const { invalid } = parseQuantAssumption(value, positive);
  const errorId = useId();
  const error = invalid ? (positive ? "Nhập số nguyên lớn hơn 0." : "Nhập số nguyên từ 0 trở lên.") : semanticError;
  return <label className="block text-xs text-[#b8cac3]">{label}<Input type="text" inputMode="numeric" value={value} aria-label={label} aria-invalid={error !== null} aria-describedby={error ? errorId : undefined} onChange={(event) => onChange(event.target.value)} className="mt-1 h-9 rounded-[4px] border-white/10 bg-black/25 text-sm" />{error && <span id={errorId} role="alert" className="mt-1 block text-rose-200">{error}</span>}</label>;
}

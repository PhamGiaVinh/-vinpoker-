...
import { buildOpsQuantDashboardQ1, explainQuantArtifact, parseQuantAssumption, selectQuantEvent, type QuantDraftQ1, type QuantSourceReceiptsQ1, type OpsQuantDashboardQ1Model, type QuantExplanationKey, type QuantPressureStatus, type QuantTruthClass } from "./opsQuantDashboardQ1";
...
export function OpsQuantDashboardQ1View({ clubId, clubName, draft, onDraftChange, receipts, onReceiptsChange }: {
  ...
  const customGtd = customGtdInput.value;
  const customPeakConcurrentPlayers = parseQuantAssumption(draft.customPeakConcurrentPlayers).value;
  ...
  useEffect(() => {
    ...
    if (!operations.isError && operations.data && operations.data.availability !== "unavailable") accepted["live-operations"] = { asOf: operations.data.asOf, observedAt: operations.data.observedAt };
    if (registrationValue && registration.data) accepted["registration-q0"] = { asOf: registrationValue.asOf, observedAt: registration.data.observedAt };
    if (sepayValue && sepay.data) accepted["sepay-q0"] = { asOf: sepayValue.asOf, observedAt: sepay.data.observedAt };
    if (!series.isError && series.data?.value) accepted["series-history"] = { asOf: null, observedAt: series.data.observedAt };
    if (!prizePool.isError && prizePool.data?.value && selectedEventId) accepted[`true-prize-pool:${selectedEventId}`] = { asOf: null, observedAt: prizePool.data.observedAt };
    onReceiptsChange((previous) => Object.entries(accepted).every(([key, value]) => previous[key]?.observedAt === value.observedAt && previous[key]?.asOf === value.asOf) ? previous : { ...previous, ...accepted });
  }, [pulseValue, pulse.data, operations.data, operations.isError, registrationValue, registration.data, sepayValue, sepay.data, series.data, series.isError, prizePool.data, prizePool.isError, selectedEventId, onReceiptsChange]);
  ...
  const model = useMemo(() => buildOpsQuantDashboardQ1({
    ...
    sepay: sepayValue,
    sepayAvailability,
    seriesEvents: series.isError ? [] : series.data?.value ?? [],
    seriesAvailability: series.isError ? "unavailable" : series.data?.availability ?? "unavailable",
    truePrizePool: prizePool.isError ? null : prizePool.data?.value ?? null,
    prizePoolAvailability: prizePool.isError ? "unavailable" : prizePool.data?.availability ?? "unavailable",
    seatsPerTable,
    customEntries,
    customGtd,
    customGtdInvalid: customGtdInput.invalid,
    customPeakConcurrentPlayers,
    forecastOptions: { calendarFeatures: FEATURES.seriesCalendarFeatures, censoring: FEATURES.seriesCensoring },
  }), [customEntries, customGtd, customGtdInput.invalid, customPeakConcurrentPlayers, operationValue, prizePool.data, prizePool.isError, pulseValue, registrationValue, registrationAvailability, requestedEventId, seatsPerTable, sepayValue, sepayAvailability, series.data, series.isError]);

  const explanation = explanationKey ? explainQuantArtifact(model, explanationKey) : null;
  const selectionMessage = registration.isPending ? "Đang tải lịch CLB…" : !registrationValue ? "Không đọc được lịch CLB. Thử Refresh hoặc xem Data Health." : requestedEventId && !model.selectedEvent ? "Giải đã chọn không còn khả dụng trong nguồn hiện tại. Hãy chọn lại; hệ thống không tự đổi giải." : !model.eventOptions.length ? "Nguồn đã đọc thành công: không có giải trong khoảng Q0." : !model.selectedEvent ? "Chọn một giải để xem Quant." : null;
  ...
  return <section className="space-y-3" data-testid="ops-quant-dashboard-q1">
    <div className="grid gap-2 border border-white/10 bg-[#050b0d] p-3 xl:grid-cols-[minmax(180px,.7fr)_minmax(260px,1.25fr)_1fr_auto] xl:items-end">
      ...
      <div className="grid grid-cols-2 gap-3 text-right"><LocalClock timezone={pulseValue?.timezone ?? null} /><HeaderStat label="Status" value={terminalStatus} tone={terminalStatus === "LIVE" ? "good" : "warn"} /></div>
    ...
    {selectionMessage && <p role="status" className="border border-amber-300/20 p-3 text-sm text-amber-100">{selectionMessage}</p>}
    ...
    <div className="grid gap-3 xl:grid-cols-[minmax(0,2.2fr)_minmax(0,5fr)_minmax(0,2.4fr)_minmax(0,2.4fr)]">
      ...
      <Panel title="Demand & turnout forecast" detail="Đăng ký quan sát · dự báo tổng cuối giải riêng">
      ...
      <Panel title="Alerts" detail={`${model.alerts.length} tín hiệu có bằng chứng`} compact>
        <AlertRail model={model} />
      </Panel>
    </div>

    <Panel title="Tournament pressure matrix" detail="Event demand · GTD stress · confidence authority">
      <PressureMatrix model={model} onSelect={changeEvent} emptyMessage={selectionMessage} />
    </Panel>

    <div className="grid grid-cols-12 gap-3">
      <Panel className="col-span-12 xl:col-span-4" title="Capacity & table coverage" detail="Phân bổ event · không phải roster cả ca">
        <CapacityPanel model={model} draft={draft} onDraftChange={onDraftChange} />
      </Panel>
      ...
      <Panel className="col-span-12 xl:col-span-4" title="Scenario lab" detail="Local assumptions · not saved">
        <ScenarioPanel model={model} draft={draft} onDraftChange={onDraftChange} />
      </Panel>
    </div>
    <details className="border border-white/10 bg-[#050b0d] p-3 text-xs text-[#91a49b]">
      <summary className="cursor-pointer text-sm text-white">Nguồn và thời điểm đọc thành công</summary>
      <div className="mt-3 space-y-2">{model.sourceHealth.map((source) => {
        const receipt = receipts[source.sourceId === "true-prize-pool" ? `true-prize-pool:${selectedEventId}` : source.sourceId];
        return <div key={source.sourceId} data-testid={`receipt-${source.sourceId}`} className="grid gap-1 border-t border-white/10 pt-2 sm:grid-cols-3"><span>{source.sourceId} · {source.availability}</span><span>Source asOf: {receipt?.asOf ?? "Không được nguồn cung cấp"}</span><span>Lần đọc thành công: {receipt?.observedAt ?? "Chưa có"}</span></div>;
      })}</div>
    </details>
...
function SignalPipeline({ model }: { model: OpsQuantDashboardQ1Model }) {
  const items = [
    ["Observed demand", model.selectedEvent?.registration.confirmedEntries ?? null, model.selectedEvent ? "OBSERVED" as const : "UNAVAILABLE" as const],
    ["Turnout forecast", model.forecast.center, model.forecast.truth],
    ["Custom table demand", model.scenarios.find((item) => item.scenarioId === "custom")?.requiredTables ?? null, "HYPOTHESIS" as const],
    ["Custom coverage gap", model.scenarios.find((item) => item.scenarioId === "custom")?.additionalDealerNeed ?? null, "HYPOTHESIS" as const],
    ["Ngưỡng entries", model.economics.requiredEntries.value, model.economics.requiredEntries.truth],
    ["Owner decision", null, "UNAVAILABLE" as const],
  ];
  return <ol className="space-y-1">{items.map(([label, rawValue, truth], index) => <li key={label as string} className="relative grid grid-cols-[20px_1fr_auto] items-center gap-2 border border-white/8 bg-black/15 px-2 py-1.5"><span className="flex h-5 w-5 items-center justify-center border border-cyan-300/20 bg-cyan-300/5 font-mono text-[9px] text-cyan-200">{index + 1}</span><span className="text-xs font-medium text-[#c4d1cd]">{label as string}<br /><TruthBadge truth={rawValue === null ? "UNAVAILABLE" : truth as QuantTruthClass} /></span><b className="font-mono text-xs text-white">{typeof rawValue === "number" ? formatNumber(rawValue) : "—"}</b></li>)}</ol>;
}

function ChartLegend({ model }: { model: OpsQuantDashboardQ1Model }) { return <div className="mb-1 flex flex-wrap gap-4 font-mono text-xs text-[#78908a]"><span className="text-cyan-200">Observed cumulative</span><span>Current {model.selectedEvent?.registration.confirmedEntries ?? "—"}</span></div>; }
...
function AlertRail({ model }: { model: OpsQuantDashboardQ1Model }) {
  const [expanded, setExpanded] = useState(false);
  const alerts = expanded ? model.alerts : model.alerts.slice(0, 2);
  return <div className="space-y-2">{alerts.map((alert) => <article key={alert.alertId} className="border border-white/8 p-2"><h3 className="text-xs font-medium text-white"><AlertTriangle className="mr-1 inline h-3.5 w-3.5 text-amber-200" />{alert.title}</h3><p className="mt-1 text-xs text-[#91a49b]">{alert.detail}</p><TruthBadge truth={alert.truth} /></article>)}{model.alerts.length > 2 && <Button type="button" variant="ghost" size="sm" className="w-full text-xs" aria-expanded={expanded} onClick={() => setExpanded(!expanded)}>{expanded ? "Thu gọn" : `Xem tất cả (${model.alerts.length})`}</Button>}{!model.alerts.length && <p className="text-xs text-[#91a49b]">Không có alert đủ evidence.</p>}</div>;
}

function PressureMatrix({ model, onSelect, emptyMessage }: { model: OpsQuantDashboardQ1Model; onSelect: (id: string) => void; emptyMessage: string | null }) {
  return <div className="overflow-x-auto"><table className="min-w-[840px] w-full text-left text-xs"><thead className="border-b border-white/8 text-[#91a49b]"><tr>{["Event", "Start", "State", "Confirmed", "Center", "Required", "GTD stress", "Authority"].map((label) => <th key={label} className="px-2 py-2">{label}</th>)}</tr></thead><tbody>{model.pressureRows.map((row) => <tr key={row.eventId} className={`border-b border-white/6 ${row.eventId === model.selectedEvent?.eventId ? "bg-cyan-300/[0.035]" : ""}`}><td className="px-2 py-2"><button type="button" className="min-h-9 text-left text-cyan-200 underline focus-visible:outline focus-visible:outline-2" onClick={() => onSelect(row.eventId)}>{row.eventName}</button></td><td className="px-2 py-2">{formatDate(row.startTime)}</td><td className="px-2 py-2">{row.state}</td><td className="px-2 py-2 font-mono">{formatNumber(row.confirmedEntries)}</td><td className="px-2 py-2 font-mono">{nullableNumber(row.center)}</td><td className="px-2 py-2 font-mono">{nullableNumber(row.requiredEntries)}</td><td className="px-2 py-2"><PressureBadge status={row.gtdStatus} /></td><td className="px-2 py-2">{row.forecastStatus} · N={row.sampleSize}</td></tr>)}</tbody></table>{!model.pressureRows.length && <p className="py-4 text-sm text-[#91a49b]">{emptyMessage ?? "Không có dữ liệu để hiển thị."}</p>}</div>;
}

function CapacityPanel({ model, draft, onDraftChange }: { model: OpsQuantDashboardQ1Model; draft: QuantDraftQ1; onDraftChange: Dispatch<SetStateAction<QuantDraftQ1>> }) {
  const custom = model.scenarios.find((item) => item.scenarioId === "custom");
  return <div><div className="grid grid-cols-3 gap-px bg-white/8"><MiniStat label="Allocated" value={model.capacity.eventAllocatedTableCount} /><MiniStat label="Dealers assigned" value={model.capacity.eventAssignedDealerCount} /><MiniStat label="Club configured" value={model.capacity.clubConfiguredTableCount} /></div><fieldset disabled={!model.selectedEvent} className="mt-3 space-y-2"><AssumptionInput label="Ghế mỗi bàn (giả định)" value={draft.seatsPerTable} positive onChange={(seatsPerTable) => onDraftChange((previous) => ({ ...previous, seatsPerTable }))} /><AssumptionInput label="Người đồng thời cao điểm (Custom)" value={draft.customPeakConcurrentPlayers} onChange={(customPeakConcurrentPlayers) => onDraftChange((previous) => ({ ...previous, customPeakConcurrentPlayers }))} /></fieldset><div className="mt-3 grid grid-cols-2 gap-2"><MiniStat label="Bàn cần theo Custom" value={custom?.requiredTables ?? null} /><MiniStat label="Dealer đứng bàn còn thiếu" value={custom?.additionalDealerNeed ?? null} /></div><div className="mt-2"><PressureBadge status={custom?.capacityStatus ?? "UNAVAILABLE"} /></div><p className="mt-2 text-xs text-[#91a49b]">HYPOTHESIS · Bao phủ bàn tại một thời điểm, không phải nhân sự cả ca. Inventory toàn CLB không phải nguồn lực rảnh đã cam kết.</p></div>;
}
...
function ScenarioPanel({ model, draft, onDraftChange }: { model: OpsQuantDashboardQ1Model; draft: QuantDraftQ1; onDraftChange: Dispatch<SetStateAction<QuantDraftQ1>> }) {
  const custom = model.scenarios.find((item) => item.scenarioId === "custom");
  const outputs = custom ? [["Entries", custom.entries], ["GTD (VND)", custom.gtd], ["Prize pool (VND)", custom.prizePool], ["Thiếu GTD (VND)", custom.overlay], ["Dư pool (VND)", custom.surplus], ["Người đồng thời", custom.peakConcurrentPlayers], ["Bàn cần", custom.requiredTables], ["Bàn còn thiếu", custom.additionalTableNeed], ["Dealer đứng bàn còn thiếu", custom.additionalDealerNeed]] as const : [];
  return <div><div className="grid grid-cols-3 gap-1.5">{model.scenarios.filter((item) => item.scenarioId !== "custom").map((scenario) => <article key={scenario.scenarioId} className="border border-fuchsia-300/15 p-2"><p className="text-xs text-fuchsia-200">{scenario.label}</p><p className="mt-2 font-mono text-lg">{nullableNumber(scenario.entries)}</p><p className="text-xs text-[#91a49b]">Gap {scenario.overlay === null ? "—" : formatVndShort(scenario.overlay)}</p></article>)}</div><fieldset disabled={!model.selectedEvent} className="mt-3 space-y-2 border-t border-cyan-300/20 pt-3"><legend className="text-sm text-cyan-200">Custom · Owner override</legend><AssumptionInput label="Custom entries" value={draft.customEntries} onChange={(customEntries) => onDraftChange((previous) => ({ ...previous, customEntries }))} /><AssumptionInput label="Custom GTD (VND)" value={draft.customGtd} onChange={(customGtd) => onDraftChange((previous) => ({ ...previous, customGtd }))} /><p className="text-xs text-[#91a49b]">{draft.customGtd.trim() ? "GTD theo owner nhập" : "GTD dùng giá trị canonical nếu có"} · LOCAL SCENARIO · NOT SAVED</p></fieldset><dl data-testid="custom-results" className="mt-3 space-y-2">{outputs.map(([label, amount]) => <div key={label} className="grid grid-cols-[1fr_auto] gap-2 border-b border-white/8 pb-1 text-xs"><dt>{label}<br /><TruthBadge truth={amount === null ? "UNAVAILABLE" : "HYPOTHESIS"} /></dt><dd className="font-mono">{nullableNumber(amount)}</dd></div>)}</dl>{!custom && <p className="mt-2 text-xs text-[#91a49b]">Chọn giải và nhập giả định để xem kết quả Custom.</p>}</div>;
}

function AssumptionInput({ label, value, positive = false, onChange }: { label: string; value: string; positive?: boolean; onChange: (raw: string) => void }) {
  const { invalid } = parseQuantAssumption(value, positive);
  const errorId = useId();
  return <label className="block text-xs text-[#b8cac3]">{label}<Input type="text" inputMode="numeric" value={value} aria-label={label} aria-invalid={invalid} aria-describedby={invalid ? errorId : undefined} onChange={(event) => onChange(event.target.value)} className="mt-1 h-9 rounded-[4px] border-white/10 bg-black/25 text-sm" />{invalid && <span id={errorId} role="alert" className="mt-1 block text-rose-200">{positive ? "Nhập số nguyên lớn hơn 0." : "Nhập số nguyên từ 0 trở lên."}</span>}</label>;
}

function LocalClock({ timezone }: { timezone: string | null }) {
  const [now, setNow] = useState(() => new Date().toISOString());
  useEffect(() => {
    const timer = window.setInterval(() => setNow(new Date().toISOString()), 60_000);
    return () => window.clearInterval(timer);
  }, []);
  return <HeaderStat label="Giờ máy tại CLB" value={formatInTimezone(now, timezone)} />;
}

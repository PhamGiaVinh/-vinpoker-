const FLOOR_ERROR_MESSAGES: Record<string, string> = {
  ...
  actor_mismatch: "Phiên thao tác không khớp tài khoản đang đăng nhập.",
  ...
  tournament_cancelled: "Giải đã huỷ nên không thể chốt.",
  ...
  game_table_in_use: "Bàn vật lý đang được một giải khác sử dụng.",
  table_has_active_seats: "Bàn đóng vẫn còn ghế hoạt động. Cần kiểm tra dữ liệu trước khi mở lại.",
...
export function floorOpsErrorMessage(code: string | null | undefined, fallback = "Thao tác thất bại"): string {
  ...
  return FLOOR_ERROR_MESSAGES[code] ?? `${fallback} (${code})`;
}

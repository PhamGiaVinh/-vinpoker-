...
export function PrizesPanel({ tournamentId, rpt = false }: { tournamentId: string; rpt?: boolean }) {
  ...
  return (
    <div className="min-w-0 space-y-2.5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-border/50 bg-card/50 px-3.5 py-2 text-[12px]">
      ...
      <div className="overflow-hidden rounded-xl border border-border/50">
        <table className="w-full table-fixed text-sm">
          <colgroup><col className="w-16" /><col /><col className="w-28 sm:w-36" /></colgroup>
          <thead>
            ...
            {rows.map((r) => {
              ...
              return (
                <tr key={r.position} className="border-t border-border/30">
                  ...
                  <td className="break-words px-3 py-2 text-foreground [overflow-wrap:anywhere]">{who || <span className="text-muted-foreground/60">—</span>}</td>

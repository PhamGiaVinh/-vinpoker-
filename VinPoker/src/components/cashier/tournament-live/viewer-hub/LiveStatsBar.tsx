...
export function LiveStatsBar({ prizePool, playersRemaining, chipLeader, rpt = false }: LiveStatsBarProps) {
  ...
  const prizeTone = rpt ? "text-[hsl(var(--viewer-neon))]" : "text-warning";
  const leaderTone = rpt ? "hsl(var(--viewer-neon))" : "hsl(var(--poker-accent))";
  const leaderToneVar = rpt ? "var(--viewer-neon)" : "var(--poker-accent)";
  ...
  return (
    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
      {hasPrize && (
        <div className="rounded-xl border border-border/50 bg-card/50 px-3 py-2.5 shadow-[0_0_18px_rgba(0,0,0,0.18)]">
          <div className="tracker-display flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
            <Trophy className={`h-3.5 w-3.5 ${prizeTone}`} aria-hidden="true" /> {t("liveHub.stats.prize", "Giải thưởng")}
          </div>
          <div className={`tracker-num mt-0.5 text-lg font-bold leading-tight ${prizeTone}`}>
      ...
      {hasPlayers && (
        <div className="rounded-xl border border-border/50 bg-card/50 px-3 py-2.5 shadow-[0_0_18px_rgba(0,0,0,0.18)]">
          <div className="tracker-display flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
            <Users className="h-3.5 w-3.5 text-success" aria-hidden="true" /> {t("liveHub.stats.remaining", "Còn lại")}
          </div>
          <div className="tracker-num mt-0.5 text-lg font-bold leading-tight text-foreground">
            {playersRemaining}
            <span className="ml-1 text-xs font-normal text-muted-foreground">{t("liveHub.stats.remainingUnit", "người")}</span>
          </div>
        </div>
      )}
      ...
      {hasLeader && (
        <div className="col-span-2 rounded-xl border px-3 py-2.5 shadow-[0_0_18px_rgba(0,0,0,0.18)] sm:col-span-1"
          style={{ borderColor: `hsl(${leaderToneVar} / 0.4)`, background: `hsl(${leaderToneVar} / 0.06)` }}>
          <div className="tracker-display flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
            <Crown className="h-3.5 w-3.5" aria-hidden="true" style={{ color: leaderTone }} /> {t("liveHub.stats.chipLeader", "Chip Leader")}
          </div>
          <div className="mt-0.5 flex items-baseline gap-1.5">
            <span className="truncate text-sm font-bold text-foreground">{chipLeader!.playerName}</span>
            <span className="shrink-0 text-[10px] text-muted-foreground">{t("liveHub.seat", "Ghế {{n}}", { n: chipLeader!.seatNumber })}</span>
          </div>
          <div className="tracker-num text-xs font-semibold" style={{ color: leaderTone }}>

...
export default function SwingTableActions(p: SwingTableActionsProps) {
  if (p.mode === "empty") {
    return (
      <div className="pt-1">
        <LiquidButton size="sm" variant="outline" className={`w-full ${PRIMARY_H} text-sm text-primary`} onClick={p.onAssign}>
    ...
    <div className="pt-1 space-y-2">
      ...
      <div className="flex gap-2">
        <span className="flex-[2]" title={p.disabledReason}>
          <LiquidButton size="sm" variant="outline"
            className={[
        ...
        <LiquidButton size="sm" variant="outline" className={`flex-1 ${PRIMARY_H} text-sm`}
      ...
      <div className="flex gap-2">
        ...
        {p.wrongTableEnabled ? (
          ...
          <span className="flex-1" title="Sắp ra mắt — cần bật Room Reconcile trước">
            <LiquidButton size="sm" variant="outline" className={`w-full ${CORR_H} text-xs text-muted-foreground border-border`} disabled>
              <AlertTriangle className="w-4 h-4 mr-1.5" />
              <span className="hidden sm:inline">Sửa nhầm bàn</span>
              <span className="sm:hidden">Nhầm bàn</span>
            </LiquidButton>

    ...
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      ...
      >
        ...
        <DropdownMenuItem
          ...
          className="cursor-pointer text-xs text-red-400 focus:text-red-400 focus:bg-red-500/10"
        >
        ...
        {!loading && !error && history?.map((entry) => (
          <div key={entry.id} className="px-2 py-1.5 border-b border-border/30 last:border-0">
            <div className="flex items-center gap-1.5">
              <span className="text-xs font-medium text-amber-100/90">{entry.dealerName}</span>
              {entry.wasOT && (
                <span className="text-[9px] font-bold px-1 py-[1px] rounded-sm bg-red-500/20 text-red-400">OT</span>
              )}
            </div>
            <div className="flex justify-between mt-0.5">
              ...
              <span className="text-[10px] text-muted-foreground">
                ...
                {entry.wasOT && entry.overtimeMinutes > 0 && (
                  <span className="text-red-400 ml-1">+{entry.overtimeMinutes}OT</span>
                )}
              </span>
            </div>
          </div>

...
import { icmEquities } from "@/lib/icmEquities";
...
export default function ICMCalculator() {
  ...
  const calculate = () => {
    setResults(null);
    const chipsArr = stacks.map((s) => Number(s));
    const prizeArr = payouts.map((p) => p.trim() === "" ? 0 : Number(p));
    const totalChips = chipsArr.reduce((a, b) => a + b, 0);
    const totalPrizes = prizeArr.reduce((a, b) => a + b, 0);
    if (chipsArr.some((c) => !Number.isFinite(c) || c <= 0) || !Number.isFinite(totalChips)) {
      toast.error(t("icmCalc.errChips"));
      return;
    }
    if (prizeArr.some((p) => !Number.isFinite(p) || p < 0) || !Number.isFinite(totalPrizes) || totalPrizes <= 0) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-border/60 bg-card/40 backdrop-blur-sm p-3 sm:p-5 md:p-6 shadow-lg space-y-5">
            ...
            {PLAYER_OPTIONS.map((n) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "h-10 w-10 rounded-md border text-xs sm:text-sm font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary",
                    active
        ...
        <p className="text-xs text-muted-foreground sm:hidden">Kéo ngang để xem đủ stack và giá trị ICM.</p>
        <div role="region" aria-label="Bảng tính ICM" tabIndex={0} className="overflow-x-auto rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
        <div className="grid min-w-[32rem] grid-cols-3 gap-2 sm:min-w-0 sm:gap-4">
          {/* Payouts */}

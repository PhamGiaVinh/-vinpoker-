...
export function MonteCarloPanel({
  ...
  // geomean prize → adaptive GTD slider bounds + default
  const meanLog = selected && selected.observedEntries.length
    ...
    : 0;
  const geoPrize = selected ? Math.exp(meanLog) * selected.buyinPrize : 0;
        ...
        <>
          ...
          <div>
            <div className="flex items-center justify-between text-[11px] text-[var(--mut)] font-sans mb-1">
              ...
              <span>n={nEff} · vùng đỏ = overlay</span>
            </div>

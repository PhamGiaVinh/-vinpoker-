 ...
 */
export function TurnoutForecastPanel({ csvEvents }: { csvEvents: SeriesEvent[] }) {
      ...
      <div>
        <h3 className="font-display text-base flex items-center gap-2">
          ...
          <span className="inline-flex items-center gap-0.5 rounded-full border border-warning/50 bg-warning/10 px-1.5 py-0.5 text-[9px] text-warning">
            <FlaskConical className="h-2.5 w-2.5" /> Hypothesis
          </span>
        </h3>
        <p className="text-[11px] text-muted-foreground">
          Mô hình hồi quy minh bạch (ridge log-linear) trên giải đã chạy của CLB. <strong>Chưa phải Model Estimate</strong> —
          chỉ là khung dự báo tham khảo, gắn dải + độ tin theo lượng data. Phase 5 (mô hình chính thức) vẫn khoá.
        </p>
      </div>
      ...
      {!ready ? (
        ...
        <div className="space-y-2">
          ...
          <Card className="p-3 border-primary/40 gradient-card space-y-2 text-[11px]">
            ...
            <p className="text-[10px] text-muted-foreground/80">Mô phỏng overlay <strong>TỪ DỰ ĐOÁN</strong> (forecast-centered), không phải phân phối quan sát lịch sử. Dùng engine overlay sẵn có (read-only).</p>

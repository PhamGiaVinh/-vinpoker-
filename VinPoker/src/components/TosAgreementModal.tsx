...
const TOS_VI = `# ĐIỀU KHOẢN DỊCH VỤ VINBACKER

**Phiên bản:** 2.0  
**Nhà cung cấp:** VinBacker (sau đây gọi là "VinBacker", "Nền tảng", "chúng tôi")

## PHẦN 1. ĐỊNH NGHĨA VÀ PHẠM VI DỊCH VỤ

### Điều 1. Bản chất Nền tảng
VinBacker là **phần mềm quản lý thông tin sự kiện tập huấn thể thao trí tuệ** (SaaS), cung cấp cho các Câu lạc bộ thể thao có giấy phép hoạt động hợp pháp.

VinBacker **KHÔNG PHẢI LÀ**: dịch vụ tài chính/ngân hàng/ví điện tử; trung gian thanh toán; tổ chức cá cược/đánh bạc; sàn giao dịch chứng khoán/crypto; bên tham gia giao dịch tài chính giữa người dùng.

VinBacker **CHỈ LÀ** công cụ kỹ thuật ghi nhận thông tin hỗ trợ chi phí tham gia sự kiện tập huấn, tính toán tỷ lệ chia sẻ kết quả theo thỏa thuận dân sự tự nguyện, và hỗ trợ CLB quản lý sự kiện.
...
VinBacker không mở/quản lý/nắm giữ tài khoản thanh toán/ví điện tử/escrow nào. Mọi chuyển tiền thực hiện **trực tiếp ngoài Nền tảng** qua chuyển khoản ngân hàng hoặc tiền mặt hợp pháp tại Việt Nam.

## PHẦN 3. QUY TRÌNH HOẠT ĐỘNG

### Điều 6. Vai trò của VinBacker
...
### Điều 9. Phí cố định
Khi check-in, Player có thể thanh toán Phí nền tảng cố định cho VinBacker (qua CLB), tính theo phân khúc lệ phí tập huấn, công khai trên Nền tảng.

### Điều 10. Phí thành tích (1%)
Khi Player đạt phần thưởng, VinBacker trích **1%** trên tổng phần thưởng trước khi 99% còn lại được chia theo tỷ lệ đã thỏa thuận.
...
1. Người dùng có trách nhiệm tự bảo vệ thông tin đăng nhập (mật khẩu, email, thiết bị). VinBacker không chịu trách nhiệm về tổn thất phát sinh từ việc người dùng để lộ mật khẩu, truy cập từ thiết bị không an toàn, hoặc bị đánh cắp tài khoản do lỗi chủ quan.
...
3. VinBacker có quyền từ chối yêu cầu khôi phục tài khoản nếu người dùng không thể chứng minh quyền sở hữu email/SĐT đã đăng ký.
...
### Điều 16. Sở hữu
Mã nguồn, giao diện, thuật toán, CSDL là tài sản của VinBacker. Người dùng chỉ được dùng cho mục đích hợp pháp.
...
### Điều 20. Giới hạn trách nhiệm VinBacker
...
1. VinBacker cung cấp kênh hỗ trợ kỹ thuật và tiếp nhận phản ánh thông qua tài khoản hỗ trợ chính thức trên Nền tảng (chat) hoặc email support@vinpoker.com. Đây là kênh hỗ trợ về vận hành phần mềm, không phải kênh giải quyết tranh chấp tài chính giữa các bên.
2. Đối với tranh chấp phát sinh giữa Player và Backer (ví dụ: không đồng ý về kết quả thành tích, chậm thanh toán, vi phạm thỏa thuận), VinBacker chỉ đóng vai trò **cung cấp thông tin ghi nhận** (audit log, ảnh chứng minh, lịch sử trạng thái) để các bên tự hòa giải. VinBacker không có thẩm quyền phán xét, không bắt buộc bất kỳ bên nào thực hiện nghĩa vụ tài chính.
3. Người dùng có thể gửi khiếu nại về lỗi kỹ thuật của Nền tảng (ví dụ: tính toán sai số liệu, không hiển thị thông tin). VinBacker sẽ kiểm tra và khắc phục trong thời hạn hợp lý (tối đa 07 ngày làm việc).
4. Nếu phát hiện hành vi gian lận, sử dụng tài khoản ảo, hoặc lạm dụng Nền tảng, VinBacker có quyền đình chỉ tài khoản, xóa nội dung vi phạm và từ chối cung cấp dịch vụ mà không cần thông báo trước.
5. Mọi yêu cầu bồi thường thiệt hại đối với VinBacker chỉ được chấp nhận nếu thiệt hại đó được chứng minh là do lỗi cố ý hoặc sơ suất nghiêm trọng của VinBacker trong việc vận hành phần mềm. VinBacker không bồi thường các thiệt hại gián tiếp, thiệt hại do quyết định đầu tư/hỗ trợ chi phí của người dùng.
...
### Điều 24. Chấm dứt
VinBacker có quyền đình chỉ tài khoản vi phạm. Người dùng có quyền ngừng dùng bất cứ lúc nào. Nghĩa vụ phát sinh trước đó vẫn còn hiệu lực.
...
const TOS_EN = `# VINBACKER TERMS OF SERVICE

**Version:** 2.0  
**Provider:** VinBacker (hereinafter "VinBacker", the "Platform", "we")

## PART 1. DEFINITIONS AND SCOPE

### Article 1. Nature of the Platform
VinBacker is **SaaS software for managing intellectual sports training events**, provided to legally licensed sports clubs.

VinBacker is **NOT**: a financial/banking/e-wallet service; a payment intermediary; a betting/gambling operator; a securities/crypto exchange; or a party to financial transactions between users.

VinBacker is **ONLY** a technical tool that records cost-support information for training events, calculates result-sharing ratios under voluntary civil agreements, and helps clubs manage events.
...
VinBacker does not open/manage/hold any payment account, e-wallet, or escrow. All money transfers happen **off-platform** via legal Vietnamese bank transfer or cash.

## PART 3. OPERATING PROCESS

### Article 6. VinBacker's role
...
### Article 9. Fixed fee
At check-in, the Player may pay a fixed Platform fee to VinBacker (via the Club), tiered by training fee bracket and disclosed publicly.

### Article 10. Performance fee (1%)
When the Player wins a reward, VinBacker takes **1%** of the gross reward before the remaining 99% is split per the agreed ratio.
...
1. Users are responsible for protecting their own login credentials (password, email, devices). VinBacker is not liable for losses arising from disclosed passwords, access from unsafe devices, or account theft caused by user negligence.
...
3. VinBacker reserves the right to refuse a recovery request if the user cannot prove ownership of the registered email/phone.
...
Source code, UI, algorithms, and database belong to VinBacker. Users may only use them lawfully.
...
1. VinBacker provides technical support and feedback channels via the official support account on the Platform (chat) or email support@vinpoker.com. These are software-operations support channels, not channels for resolving financial disputes between parties.
2. For disputes between Player and Backer (e.g., disagreements about results, late payments, breach of agreement), VinBacker only acts to **provide recorded information** (audit logs, proof images, status history) for the parties to settle on their own. VinBacker has no adjudication authority and does not compel any party to perform financial obligations.
3. Users may submit complaints about Platform technical errors (e.g., incorrect calculations, missing information). VinBacker will review and remediate within a reasonable time (up to 7 working days).
4. If fraud, fake accounts or platform abuse is detected, VinBacker may suspend the account, remove infringing content and refuse service without prior notice.
5. Compensation claims against VinBacker will only be accepted if the damages are proven to result from VinBacker's intentional fault or gross negligence in operating the software. VinBacker does not compensate indirect damages or damages from users' investment/cost-support decisions.
...
const TOS_ZH = `# VINBACKER 服务条款

**版本:** 2.0  
**提供方:** VinBacker (以下简称 "VinBacker"、"平台"、"我们")

## 第一部分 定义与服务范围

### 第1条 平台性质
VinBacker 是面向已合法持牌的体育俱乐部提供的**智力体育训练赛事信息管理 SaaS 软件**。

VinBacker **不是**: 金融/银行/电子钱包服务; 支付中介; 博彩/赌博机构; 证券/加密货币交易所; 也不是用户之间金融交易的当事方。

VinBacker **仅是**记录训练赛事费用支持信息、按自愿民事约定计算结果分享比例、并协助俱乐部管理赛事的技术工具。
...
### 第5条 非支付中介
VinBacker 不开设/管理/持有任何支付账户、电子钱包或托管账户。所有资金转移均**在平台之外**通过越南合法的银行转账或现金完成。
...
### 第9条 固定费用
签到时,Player 可向 VinBacker(经俱乐部)缴纳固定的平台费,按训练费分档收取并公开披露。

### 第10条 成绩费 (1%)
Player 取得奖励时,VinBacker 在按约定比例分配剩余 99% 之前,先抽取奖励总额的 **1%**。
...
### 第15条 账户恢复与安全
1. 用户应自行妥善保管登录凭证(密码、邮箱、设备)。因密码泄露、在不安全设备登录或主观过失导致账户被盗所产生的损失,VinBacker 不承担责任。
2. 忘记密码时,用户可通过注册邮箱申请重置密码。重置链接的有效期由系统规定(通常为 60 分钟),且仅发送至本人注册邮箱。
3. 若用户无法证明对注册邮箱/手机号的所有权,VinBacker 有权拒绝账户恢复请求。
4. 用户承诺不使用他人账户,不共享登录会话(session),不使用自动化工具访问平台。

## 第六部分 知识产权与数据

### 第16条 所有权
源代码、界面、算法、数据库归 VinBacker 所有。用户仅可合法使用。
...
### 第22条 支持与投诉
1. VinBacker 通过平台上的官方支持账户(聊天)或邮箱 support@vinpoker.com 提供技术支持与反馈接收渠道。这是软件运营支持渠道,不是各方之间财务争议的处理渠道。
2. 对于 Player 与 Backer 之间的争议(例如对成绩结果有异议、付款延迟、违反约定),VinBacker 仅**提供留存信息**(审计日志、凭证图片、状态历史),供双方自行协商解决。VinBacker 无裁决权,亦不强制任何一方履行财务义务。
3. 用户可就平台技术故障(例如计算错误、信息显示异常)提交投诉。VinBacker 将在合理期限内核查并处理(最长 7 个工作日)。
4. 如发现欺诈、虚假账户或滥用平台行为,VinBacker 有权暂停账户、删除违规内容并拒绝提供服务,无需事先通知。
5. 仅在能够证明损害是由 VinBacker 在软件运营中故意或重大过失造成的情况下,方可受理对 VinBacker 的赔偿请求。VinBacker 不对间接损失,以及用户的投资/费用支持决策所致的损失承担赔偿责任。
...
export const TosAgreementModal = ({ open, onOpenChange, onAgree }: Props) => {
  ...
  const titleText = lang === "en"
    ? "VinBacker Terms of Service & Privacy Policy"
    : lang === "zh"
      ? "VinBacker 服务条款与隐私政策"
      : "Điều khoản Dịch vụ & Chính sách Bảo mật VinBacker";

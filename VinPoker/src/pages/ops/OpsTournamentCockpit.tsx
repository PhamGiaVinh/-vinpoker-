...
export default function OpsTournamentCockpit() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data, error } = await supabase.rpc("get_tournament_leaderboard", { p_tournament_id: id });
        ...
        const candidate = Array.isArray(payload)
          ...
          : payload && typeof payload === "object" && "players" in payload
            ...
            : [];
        const rows = (Array.isArray(candidate) ? candidate : []) as TournamentLeaderboardPlayer[];
  ...
  const doRestore = useCallback(async () => {
    ...
    setRestoreBusy(true);
    try {
      const { data, error } = await supabase.rpc("restore_busted_player_to_seat", {
      ...
      if (error || !res?.ok) {
        ...
        toast.error(floorOpsErrorMessage(code, "Khôi phục thất bại")); return;
      }
  ...
  }, [restoreTarget, restoreTtId, restoreSeat, restoreBusy, restoreConfirmed, user, floor]);
      ...
      {tab === "players" && (cockpitOn ? (() => {
        ...
        return (
          <div className="space-y-2">
            ...
            {busted.error ? (
              <CenterCard icon={<AlertTriangle className="h-7 w-7 text-rose-300" />} title="Không tải được người đã loại" sub="Không dùng dữ liệu rỗng thay thế. Hãy tải lại trang." />
